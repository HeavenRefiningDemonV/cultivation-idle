...
const toStringRecord = (value) => isRecord(value)
...
const normalizeHeartLawState = (value) => {
...
const normalizeTechniqueState = (value) => {
    const record = isRecord(value) ? value : {};
...
const normalizeMedicinePouchState = (value) => {
    const record = isRecord(value) ? value : {};
    const slots = isRecord(record.slots) ? record.slots : {};
    return {
        ...record,
        slots: Object.fromEntries(Object.entries(slots).map(([slotKey, slotValue]) => {
            const slot = isRecord(slotValue) ? slotValue : {};
            return [slotKey, { ...slot, equippedItemId: null, lastUsedAt: null }];
        })),
    };
};
const normalizeCraftSessionState = (value) => {
    const record = isRecord(value) ? value : {};
    return {
        ...record,
...
const normalizeExpeditionState = (value) => {
    const record = isRecord(value) ? value : {};
    return {
        ...record,
...
const normalizeEquipmentState = (value) => ({
...
const normalizeManualSatchelState = (value) => {
    const record = isRecord(value) ? value : {};
    return {
        ...record,
...
const normalizeActivityState = (value) => ({
    ...(isRecord(value) ? value : {}),
    active: null,
    lastChangedAt: null,
    history: [],
});
const normalizeOutskirtsState = (value) => ({
    ...(isRecord(value) ? value : {}),
    progressByOutskirtsId: {},
    autoContinue: true,
    stopAtBoss: false,
});
const normalizeProfessionState = (value) => ({
    ...(isRecord(value) ? value : {}),
    alchemyQueue: [],
    talismanQueue: [],
    forgeQueue: [],
...
export const applyPartialResetResidueCleanup = (save) => {
    ...
    const detected = {
        city: typeof cityState.currentCityId === 'string' && cityState.currentCityId !== normalizedCityState.currentCityId
        ...
        ruins: (isRecord(ruinsState.progressByRuinId) && Object.keys(ruinsState.progressByRuinId).length > 0)
            || Array.isArray(ruinsState.runHistory) && ruinsState.runHistory.length > 0
            || ruinsState.activeRun != null,
        equipment: typeof equipmentState.equippedWeaponId === 'string'
        ...
        inventory: (isRecord(inventoryState.items) && Object.keys(inventoryState.items).length > 0)
        ...
        bounty: (isRecord(bountyState.activeByCityId) && Object.keys(bountyState.activeByCityId).length > 0)
            || (isRecord(bountyState.lastRefreshAtByCityId) && Object.keys(bountyState.lastRefreshAtByCityId).length > 0),
        expedition: Array.isArray(expeditionState.active) && expeditionState.active.length > 0
            || (isRecord(expeditionState.rareProgressByKey) && Object.keys(expeditionState.rareProgressByKey).length > 0),
        profession: Array.isArray(professionState.alchemyQueue) && professionState.alchemyQueue.length > 0
        ...
        manualSatchel: Array.isArray(manualSatchelState.manuals) && manualSatchelState.manuals.length > 0
            || isRecord(manualSatchelState.entries) && Object.keys(manualSatchelState.entries).length > 0
            || manualSatchelState.activeStudy != null,
        techCollection: (isRecord(techCollectionState.unlockedTechs) && Object.keys(techCollectionState.unlockedTechs).length > 0)
            || (isRecord(techCollectionState.fragments) && Object.keys(techCollectionState.fragments).length > 0),
        heartLaw: heartLawState.selectedHeartLawId != null
            || Number(heartLawState.chapter ?? 1) > 1
            || Number(heartLawState.comprehension ?? 0) > 0
            || heartLawState.studyTechniqueId != null,
        technique: Array.isArray(techniqueState.loadouts) && techniqueState.loadouts.some((loadout) => {
            if (!isRecord(loadout) || !isRecord(loadout.slots))
                return false;
