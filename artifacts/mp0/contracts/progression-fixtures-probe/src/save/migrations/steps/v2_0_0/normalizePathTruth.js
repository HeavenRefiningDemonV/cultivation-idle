...
export const v2_0_0_normalize_path_truth = {
    ...
    priority: 30,
    appliesTo: (save, ctx) => {
        if (compareSaveVersions(ctx.sourceVersion, CURRENT_SAVE_VERSION) >= 0)
            return false;
    ...
    run: (save) => {
        ...
        const touched = [];
        let mutated = false;
        const gameState = isRecord(next.gameState) ? next.gameState : ((next.gameState = {}), next.gameState);
        ...
        if (selectedPath != null && lifePath != null && selectedPath !== lifePath) {
            warnings.push(warning('PATH_CONFLICT_RESOLVED_TO_SELECTED_PATH', `selectedPath (${selectedPath}) and legacy lifePath (${lifePath}) disagreed; selectedPath was kept.`, '0.2', 'warning', 'gameState.lifePath'));
        }
        if ('lifePath' in gameState) {
            ...
            if (lifePath != null || selectedPath != null) {
                warnings.push(warning('LEGACY_LIFEPATH_ALIAS_DROPPED', 'Legacy lifePath alias was consumed and removed from migrated save output.', '0.2', 'info', 'gameState.lifePath'));
            }
        }
        return createStepResult(v2_0_0_normalize_path_truth, next, mutated ? 'Path truth normalized to canonical selectedPath output.' : 'Path truth already normalized.', { didMutate: mutated, warnings, touchedFieldPaths: touched });
    },
};
