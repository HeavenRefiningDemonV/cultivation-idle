...
export const v2_0_0_clamp_semester_slice = {
    ...
    run: (save) => {
        ...
        if (realmState && currentRealmIndex != null && currentRealmIndex > CONTENT_CAP_INDEX) {
            ...
            touched.push(touch('gameState.realm.index', 'set', `Clamped over-cap realm index ${currentRealmIndex} to ${CONTENT_CAP_INDEX}.`), touch('gameState.realm.name', 'set', `Normalized realm display to ${CONTENT_CAP_NAME}.`));
            didMutate = true;
        }
        if (highestRealmReached != null && highestRealmReached > CONTENT_CAP_INDEX) {
            prestigeState.highestRealmReached = CONTENT_CAP_INDEX;
            touched.push(touch('prestigeState.highestRealmReached', 'set', `Clamped over-cap highest realm ${highestRealmReached} to ${CONTENT_CAP_INDEX}.`));
            didMutate = true;
        }
        return createStepResult(v2_0_0_clamp_semester_slice, next, `Normalized out-of-slice realm progress to ${CONTENT_CAP_NAME} (index ${CONTENT_CAP_INDEX}).`, {
            didMutate,
            warnings: [
                warning('OUT_OF_SLICE_PROGRESS_NORMALIZED', `Legacy save progression beyond ${CONTENT_CAP_REALM} was clamped to the live semester cap.`, '1.1', 'warning', 'gameState.realm.index'),
            ],
