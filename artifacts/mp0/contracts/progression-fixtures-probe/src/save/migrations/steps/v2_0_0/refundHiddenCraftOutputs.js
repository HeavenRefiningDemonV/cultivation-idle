...
import { getHiddenCraftRefundDefinitionByOutputItemId, getHiddenCraftRefundDefinitionBySourceId, listHiddenCraftOutputItemIds, } from '../../../../systems/economy/index.js';
...
function addItem(refunds, itemId, qty) {
    if (!itemId || qty <= 0)
        return;
    refunds.items[itemId] = (refunds.items[itemId] ?? 0) + qty;
}
function addCurrency(refunds, key, amount) {
    if (!key || amount <= 0)
        return;
    refunds.currencies[key] = (refunds.currencies[key] ?? 0) + amount;
}
function refundDefinition(refunds, sourceId, crafts) {
    if (crafts <= 0)
        return;
...
function refundHiddenInventoryItem(refunds, itemId, quantity) {
...
function collapseHiddenRefundItems(refunds) {
    ...
    while (pending.length > 0) {
        ...
        if (quantity <= 0 || !HIDDEN_OUTPUT_ITEM_IDS.has(itemId))
            continue;
        ...
        Object.entries(refunds.items).forEach(([nestedItemId, nestedQty]) => {
            if (!HIDDEN_OUTPUT_ITEM_IDS.has(nestedItemId) || nestedQty <= 0)
                return;
            const previousQty = before[nestedItemId] ?? 0;
            if (nestedQty > previousQty)
                pending.push(nestedItemId);
        });
    }
}
function applyRefunds(next, refunds) {
...
export const v2_0_0_refund_hidden_craft_outputs = {
    ...
    appliesTo: (save) => {
        ...
        const items = asRecord(inventoryState.items);
        if (Object.keys(items).some((itemId) => HIDDEN_OUTPUT_ITEM_IDS.has(itemId) && toNumber(items[itemId]) > 0))
            return true;
        const professionState = asRecord(save.professionState);
        const alchemyQueue = asArray(professionState.alchemyQueue);
        const talismanQueue = asArray(professionState.talismanQueue);
        const forgeQueue = asArray(professionState.forgeQueue);
        if (alchemyQueue.some((job) => typeof job.recipeId === 'string' && getHiddenCraftRefundDefinitionBySourceId(job.recipeId)))
            return true;
        if (talismanQueue.some((job) => typeof job.recipeId === 'string' && getHiddenCraftRefundDefinitionBySourceId(job.recipeId)))
            return true;
        if (forgeQueue.some((job) => typeof job.blueprintId === 'string' && getHiddenCraftRefundDefinitionBySourceId(job.blueprintId)))
            return true;
        ...
        if (asArray(buffState.activeTalismans).some((entry) => typeof entry.itemId === 'string' && HIDDEN_OUTPUT_ITEM_IDS.has(entry.itemId)))
            return true;
        const medicinePouchState = asRecord(save.medicinePouchState);
        const slots = asRecord(medicinePouchState.slots);
        if (Object.values(slots).some((slot) => isRecord(slot) && typeof slot.equippedItemId === 'string' && HIDDEN_OUTPUT_ITEM_IDS.has(slot.equippedItemId)))
            return true;
        return false;
    },
    run: (save) => {
        ...
        const refunds = { items: {}, currencies: {} };
        ...
        Object.keys(items).forEach((itemId) => {
            ...
            if (!HIDDEN_OUTPUT_ITEM_IDS.has(itemId) || quantity <= 0)
                return;
            refundHiddenInventoryItem(refunds, itemId, quantity);
            delete items[itemId];
        ...
        queueSpecs.forEach(([queueKey, idKey]) => {
            const queue = asArray(professionState[queueKey]);
            const kept = queue.filter((job, index) => {
                const sourceId = typeof job[idKey] === 'string' ? job[idKey] : '';
        ...
        const activeTalismans = asArray(buffState.activeTalismans);
        const keptTalismans = activeTalismans.filter((entry, index) => {
            const itemId = typeof entry.itemId === 'string' ? entry.itemId : '';
            if (!HIDDEN_OUTPUT_ITEM_IDS.has(itemId))
                return true;
        ...
        Object.entries(slots).forEach(([slotKey, rawSlot]) => {
            ...
            if (!equippedItemId || !HIDDEN_OUTPUT_ITEM_IDS.has(equippedItemId))
                return;
        ...
        if (didMutate) {
            ...
            warnings.push(warning('HIDDEN_CRAFT_OUTPUTS_REFUNDED', `Hidden craft cleanup removed inventory=${removedInventoryCount}, queueEntries=${removedQueueCount}, activeTalismans=${removedBuffCount}, medicineSlots=${removedMedicineCount}, activeSessionCleared=${clearedActiveSession}.`, '3.1', 'info', 'inventoryState.items'));
        }
        return createStepResult(v2_0_0_refund_hidden_craft_outputs, next, didMutate
            ...
            : 'No hidden craft residue detected.', { warnings, touchedFieldPaths: touched, plannedMutations, didMutate });
    },
};
