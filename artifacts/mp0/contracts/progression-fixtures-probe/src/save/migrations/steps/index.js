...
const legacyDetectionStep = {
    ...
    run: (save, ctx) => {
        ...
        if (ctx.sourceVersionKind === 'malformed-version') {
            warnings.push(warning('malformed-version', 'Save version string is malformed; falling back to 0.0.0 for compatibility migration.', '0.2', 'warning', 'version'));
        }
...
export const migrationSteps = [legacyDetectionStep, ...v2_0_0MigrationPack];
