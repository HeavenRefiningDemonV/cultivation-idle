import { buildAllCityActivityRewardReadModels, } from './activityRewardReadModel.js';
import { buildActivityThroughputCitySnapshot } from './activityThroughputReadModel.js';
import { classifyActivityRewardItem, getCityRewardRoleProfile, getDeterministicRuinAnchorItemId, getTargetedCityMaterialIds, } from './activityRewardRoles.js';
import { getOutskirtsDropsConfig, getRuinsDropsConfig } from './activityRewardRuntime.js';
const average = (range, fallback = 0) => {
    if (!range)
        return fallback;
    return (Number(range[0] ?? fallback) + Number(range[1] ?? fallback)) / 2;
};
const valueByIndex = (source, index, fallback) => {
...
function summarizeUnits(cityId, entries) {
    const summary = {
        itemUnitsPerLoop: 0,
    ...
    entries.forEach((entry) => {
        ...
        if (bucket === 'common_field')
            summary.commonUnitsPerLoop += entry.expectedQty;
        else if (bucket === 'targeted_local')
            summary.targetedUnitsPerLoop += entry.expectedQty;
        else if (bucket === 'anchor')
            summary.anchorUnitsPerLoop += entry.expectedQty;
        else if (bucket === 'support')
            summary.supportUnitsPerLoop += entry.expectedQty;
        else
            summary.otherUnitsPerLoop += entry.expectedQty;
    });
...
function buildOutskirtsMetrics(outskirts, economy) {
...
function expectedDropTableEntries(table) {
...
function buildRuinsMetrics(ruin, economy) {
...
export function inspectCityRewardParity(content, cityId) {
    ...
    targetedIds.forEach((itemId) => {
        if (commonPool.has(itemId))
            overlapRisks.push(`Outskirts common pool leaks targeted item ${itemId}`);
    });
    const anchor = getDeterministicRuinAnchorItemId(cityId);
    if (commonPool.has(anchor) || rarePool.has(anchor))
        overlapRisks.push(`Outskirts overlaps the ruin anchor ${anchor}`);
    const rules = [
        {
        ...
        },
        {
            ...
            passed: ruinsMetrics.targetedAndAnchorShare >= 0.2 &&
        ...
        },
        {
            ...
            passed: outskirtsMetrics.commonShare > ruinsMetrics.commonShare &&
...
export function buildRewardParityAuditReport(content) {
    return {
