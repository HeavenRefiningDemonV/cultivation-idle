import { getVisibleAlchemyRecipes } from './liveEconomyCatalog.js';
...
import { getAllProblemDestinationPolicies } from './problemDestinationPolicy.js';
import { classifyMaterialRoutingBand, findApothecaryStockSource, findOutskirtsSourceByItemId, findRuinSourceByItemId, getAllLiveCriticalTargetedMaterialIds, getCityById, getCityIndex, getCitySupportIdentity, getItemName, getOnePriorCityId, isBuildCorrectionItem, isSupportCurrencyId, listExpeditionItemSources, listVisibleAlchemyInputsForOutput, listVisibleAlchemyOutputItemIds, listVisibleForgeInputItemIds, getLiveCriticalBuildCorrectionItemIds, getTargetedMaterialOwningCityId, } from './economicSourceAdapters.js';
...
const LIVE_CRITICAL_SCOPE_CURRENCY_IDS = ['gold', 'merit', 'spiritStones'];
function unique(values) {
    return [...new Set(values)].sort();
}
function compareOptions(left, right) {
    const activityOrder = { active: 0, background: 1, passive: 2 };
    const localityOrder = { city_local: 0, evergreen: 1, one_prior_fallback: 2 };
    const kindOrder = {
        apothecary_buy: 0,
        apothecary_brew: 1,
        ruins: 2,
...
function getScopeTargetIds(content) {
    const targets = new Set(LIVE_CRITICAL_SCOPE_CURRENCY_IDS);
    ...
    content.cities.forEach((city) => {
        const floors = buildEconomicStockFloorSnapshot({ gateIndex: city.index + 1, cityId: city.id });
        if (floors.cultivationPrepItemId)
            targets.add(floors.cultivationPrepItemId);
    });
...
function buildCurrencyEntry(targetId) {
    if (targetId === 'gold') {
        const sourceOptions = [
            {
    ...
    if (targetId === 'merit') {
        ...
        const sourceOptions = [
            ...
            },
            {
                ...
                routeDetail: 'Eligible gate defeats provide bounded fallback Merit only when that support truth is active.',
                reason: 'Gate-defeat fallback is allowed only when the current gate support rule is active.',
                activityMode: 'active',
                locality: 'evergreen',
                classification: 'support',
...
function buildItemEntry(content, targetId) {
    ...
    const classification = isBuildCorrectionItem(targetId)
        ? 'support'
        : targetOwningCityId
            ? 'targeted'
    ...
    if (targetOwningCityId) {
        ...
        const sinkMapEntry = getAllLiveCriticalTargetedMaterialIds().includes(targetId)
            ? getTargetedMaterialSinkMapEntry(targetId)
            : null;
...
export function buildBestSourceIndex(content) {
    const scopeTargetIds = getScopeTargetIds(content);
    const entries = scopeTargetIds.map((targetId) => isSupportCurrencyId(targetId) ? buildCurrencyEntry(targetId) : buildItemEntry(content, targetId));
    const entriesByTargetId = Object.fromEntries(entries.map((entry) => [entry.targetId, entry]));
    return { scopeTargetIds, entries, entriesByTargetId };
}
export function getBestSourceIndexEntry(index, targetId) {
    return index.entriesByTargetId[targetId] ?? null;
}
export function listLiveCriticalSourceTargetIds(content) {
    return buildBestSourceIndex(content).scopeTargetIds;
}
export function getAllProblemDestinationFamilies() {
    return unique(getAllProblemDestinationPolicies().flatMap((policy) => [...policy.primaryDestinations, ...policy.secondaryDestinations]));
}
