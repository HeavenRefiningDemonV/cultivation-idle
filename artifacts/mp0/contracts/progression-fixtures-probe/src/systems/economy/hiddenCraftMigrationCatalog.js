...
function normalizeOutput(outputs) {
    ...
    if (!itemId || typeof qty !== 'number' || !Number.isFinite(qty) || qty <= 0)
        return undefined;
    return { itemId, qty: Math.max(1, Math.floor(qty)) };
}
export function listHiddenCraftRefundDefinitions() {
    const definitions = [];
...
export function getHiddenCraftRefundDefinitionBySourceId(sourceId) {
    return listHiddenCraftRefundDefinitions().find((entry) => entry.sourceId === sourceId);
}
export function getHiddenCraftRefundDefinitionByOutputItemId(itemId) {
...
export function listHiddenCraftOutputItemIds() {
    return listHiddenCraftRefundDefinitions()
        .map((entry) => entry.output?.itemId)
        .filter((itemId) => Boolean(itemId))
        .filter((itemId) => {
        const status = getItemRuntimeStatus({ id: itemId, category: itemId.startsWith('tal_') ? 'talisman' : itemId.startsWith('cons_') ? 'consumable' : itemId.startsWith('reagent_') ? 'reagent' : itemId.startsWith('item_') ? 'token' : 'material' });
