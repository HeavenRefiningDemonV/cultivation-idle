...
function toSeverity(gap, targetValue, priorityBand) {
    if (gap <= 0)
        return 'low';
    const normalized = targetValue > 0 ? gap / targetValue : gap;
    if (priorityBand <= 2 || normalized >= 0.75)
        return 'critical';
    if (priorityBand <= 4 || normalized >= 0.4)
        return 'high';
...
function getSpecialtyCoverage(snapshot, budget) {
    ...
    if (!cityId || relevantIds.length === 0) {
        return { currentValue: 0, targetValue: snapshot.spendPolicy.consumableFloor.specialtyFloor, relatedIds: [] };
    }
...
function getForgeProgressValue(snapshot, budget, recommended = false) {
    if (!budget)
    ...
    return {
        currentValue: snapshot.forgeFloor.weaponRefineFloor
            + snapshot.forgeFloor.accessoryRefineFloor
            + snapshot.forgeFloor.temperSuccessTotal
            + currentRune,
        targetValue: floor.weaponRefine
            + floor.accessoryRefine
            + floor.temperSuccesses
            + targetRune,
    };
}
function pickPrimaryRouteKey(snapshot, relatedIds) {
...
export function evaluateEconomicShortfalls(snapshot) {
    ...
    shortfalls.sort((left, right) => left.priorityBand - right.priorityBand
        || (left.mandatoryBeforeNextGate === right.mandatoryBeforeNextGate ? 0 : left.mandatoryBeforeNextGate ? -1 : 1)
        || right.gap - left.gap
        || left.label.localeCompare(right.label));
    const readinessBand = shortfalls.some((entry) => entry.mandatoryBeforeNextGate && entry.priorityBand <= 4)
        ? 'below_minimum'
