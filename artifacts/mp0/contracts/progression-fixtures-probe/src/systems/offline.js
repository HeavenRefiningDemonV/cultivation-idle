...
import { DEFAULT_OFFLINE_EFFICIENCY, formatOfflineDuration, MAX_OFFLINE_MS, MAX_OFFLINE_SECONDS, ONE_WEEK_SECONDS, resolveOfflineCultivationEfficiency, } from '../services/time/offlineShared.js';
 ...
 */
export function calculateOfflineProgress(lastOnlineUtc, currentUtc) {
...
export function buildOfflineContext(lastActiveAtMs, options) {
...
export function computeCultivationOfflineGain(dtSeconds) {
 ...
 */
export function applyOfflineProgressFromContext(context) {
    try {
 ...
 */
export function getMaxOfflineTime() {
 ...
 */
export function previewOfflineGains(hours) {
    const gameState = useGameStore.getState();
    const qiPerSecond = D(gameState.qiPerSecond);
