...
const PHASE_TARGET_MINUTES_BY_GATE = {
    1: 55,
    2: 80,
    3: 120,
    4: 170,
...
const SUPPORT_PACING_TARGETS_BY_GATE = Object.freeze(Object.fromEntries(getAllSupportReserveTargets().map((target) => {
...
export const PREP_ECONOMY_TARGETS = Object.freeze({
    ...
    isolatedRecoveryCategories: ['consumables_only', 'forge_floor_only', 'build_correction_only'],
    packageFitPolicy: {
        supplementLaneSatisfiedByAnyHonestOption: true,
        directCoreCoverageModes: ['direct', 'brew', 'hybrid', 'unsupported'],
...
export function getPrepRecoveryWindowTarget(gateIndex, kind) {
    const safe = Math.max(1, Math.min(5, Math.floor(gateIndex || 1)));
...
export function validatePrepRecoveryWindowsAgainstPhaseTargets() {
    const phaseByGate = new Map(getLockedCityPhaseTargetDurations().map((entry) => [entry.realmIndex + 1, entry.targetSeconds / 60]));
