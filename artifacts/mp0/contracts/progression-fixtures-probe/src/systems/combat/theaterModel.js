...
export function computeRollingDps(events, now, windowMs = 10000) {
...
export function computeEffectiveHp(playerHp, absorptionShield, combatShieldAmount) {
    const hp = D(playerHp);
...
export function safeDurationSeconds(ms) {
    if (!ms || ms <= 0)
        return '0.0s';
