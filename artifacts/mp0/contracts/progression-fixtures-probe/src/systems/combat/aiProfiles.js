...
export const AI_PROFILE_OPTIONS = [
 ...
 */
export function getTechniqueAiTags(def, effects) {
    const tags = (def.tags ?? []).map((tag) => tag.toLowerCase());
    const maybeEffect = def.effect;
    const fallbackEffects = Array.isArray(maybeEffect) ? maybeEffect : [];
    ...
    const isBurstDamage = classification.isUltimate ||
...
export function applyAiProfileBias(baseScore, tags, classification, context) {
    ...
    if (profile === 'survivor') {
        if (hpPct < 0.5) {
            ...
            if (tags.isShield)
                score += 5;
            if (tags.isCleanse && isPlayerDebuffed)
                score += 4;
            if (classification.isUltimate || tags.isBurstDamage)
                score -= 2;
        }
        if (hpPct < 0.3 && (tags.isHeal || tags.isShield))
            score += 2;
        if (tags.isHeal || tags.isShield)
            score += 1;
        if (classification.isBurst && hpPct < 0.55)
            score -= 1.5;
        return score;
    }
    if (profile === 'burst') {
        if (enemyIsBoss || enemyHpPct > 0.4) {
            if (tags.isBurstDamage || classification.isUltimate || classification.isBurst)
                score += 4;
            if (tags.isAoe && !enemyIsBoss)
                score += 1;
            if (tags.isHeal && hpPct > 0.75)
                score -= 1;
        }
        else {
            // Late fight: stay close to balanced but keep slight aggression
            if (classification.isBurst || tags.isBurstDamage)
                score += 1.5;
        }
        return score;
    }
    ...
    if (!enemyIsBoss) {
        ...
        if (tags.isBurstDamage)
            score += 1.5;
        if (classification.isUltimate && enemyHpPct < 0.3)
            score -= 1;
        if (tags.isHeal || tags.isShield)
            score -= 1.5;
    }
        ...
        if (tags.isBurstDamage || classification.isUltimate)
            score += 1;
        if (tags.isHeal && hpPct > 0.7)
            score -= 0.5;
    }
    return score;
}
export function buildAiReason(techniqueName, tags, context) {
