...
const CURRENT_PATH_IDS = ['heaven', 'earth', 'martial'];
function freezeMasteryTargets(targets) {
    return Object.freeze(targets.map((target) => Object.freeze({ ...target })));
}
function freezeThresholds(thresholds) {
    return Object.freeze({
        ...thresholds,
        masteryTargets: freezeMasteryTargets([...thresholds.masteryTargets]),
    });
}
function freezeGateBuildFloor(entry) {
    return Object.freeze({
...
function validateMasteryTargets(trialId, label, targets) {
    ...
    targets.forEach((target, index) => {
        ...
        if (index > 0) {
            assert(compareTargets(targets[index - 1], target) <= 0, `${trialId} ${label} mastery targets must be sorted by level ascending, then count ascending.`);
        }
    });
}
function validateThresholdPair(entry) {
    assert(entry.recommended.activeSlotsFilled >= entry.minimum.activeSlotsFilled, `${entry.trialId} recommended activeSlotsFilled cannot be weaker than minimum.`);
    assert(entry.recommended.passiveSlotsFilled >= entry.minimum.passiveSlotsFilled, `${entry.trialId} recommended passiveSlotsFilled cannot be weaker than minimum.`);
    assert(entry.recommended.pathAlignmentScore >= entry.minimum.pathAlignmentScore, `${entry.trialId} recommended pathAlignmentScore cannot be weaker than minimum.`);
    assert(entry.recommended.rankUpTotal >= entry.minimum.rankUpTotal, `${entry.trialId} recommended rankUpTotal cannot be weaker than minimum.`);
    assert(entry.recommended.runeCount >= entry.minimum.runeCount, `${entry.trialId} recommended runeCount cannot be weaker than minimum.`);
    assert(!entry.minimum.ultimateRequired || entry.recommended.ultimateRequired, `${entry.trialId} recommended ultimateRequired must stay true when minimum ultimateRequired is true.`);
}
function validatePreferredAi(trialId, preferredAiByPath) {
...
function validateRegistryEntries(registry) {
    ...
    const liveTrialIdSet = new Set(liveTrialIds);
    const seenTrialIds = new Set();
    assert(GATE_BUILD_FLOOR_TRIAL_ORDER.length === liveTrialIds.length, 'GATE_BUILD_FLOOR_TRIAL_ORDER must match the live trial count.');
    assert(GATE_BUILD_FLOOR_TRIAL_ORDER.every((trialId, index) => trialId === liveTrialIds[index]), 'GATE_BUILD_FLOOR_TRIAL_ORDER must exactly cover SEMESTER_SLICE_CONTRACT.liveTrialIds in live order.');
...
export const GATE_BUILD_FLOOR_TRIAL_ORDER = Object.freeze([
...
export const GATE_BUILD_FLOOR_REGISTRY = Object.freeze([
...
export const GATE_BUILD_FLOOR_BY_TRIAL_ID = Object.freeze(GATE_BUILD_FLOOR_REGISTRY.reduce((acc, entry) => {
    acc[entry.trialId] = entry;
    return acc;
}, {}));
export function getAllGateBuildFloors() {
    return [...GATE_BUILD_FLOOR_REGISTRY];
}
export function getGateBuildFloor(trialId) {
    if (trialId == null)
        return null;
    return GATE_BUILD_FLOOR_BY_TRIAL_ID[trialId] ?? null;
}
