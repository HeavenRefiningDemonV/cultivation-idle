 ...
 */
export function generateLoot(enemy, playerLuck, pityState, isBoss = false, isFirstBossKill = false) {
    const result = {
        items: [],
 ...
 */
export function rollLoot(drop, playerLuck) {
    const items = [];
    ...
    if (roll < adjustedDropChance) {
        const quantity = Math.floor(Math.random() * (drop.maxAmount - drop.minAmount + 1) + drop.minAmount);
        items.push({
            itemId: drop.itemId,
            quantity,
 ...
 */
function generateBossLoot(enemy, isFirstKill) {
    const items = [];
 ...
 */
export function updatePityCounters(pityState, droppedRarities) {
...
function resetPityAfterDrop(pityState, rarity) {
    const newState = { ...pityState };
    if (rarity === 'legendary' || rarity === 'mythic') {
        newState.killsSinceUncommon = 0;
        newState.killsSinceRare = 0;
 ...
 */
export function formatLootMessage(loot) {
    const messages = [];
    // Gold message
 ...
 */
export function createInitialPityState() {
    return {
