...
import { ACTIVE_PASSIVE_MASTERY_FLOOR, ACTIVE_PASSIVE_MIN_RANK_FLOOR, ACTIVE_PASSIVE_MIN_RUNE_FLOOR, ULTIMATE_MASTERY_FLOOR, ULTIMATE_MIN_RANK_FLOOR, ULTIMATE_MIN_RUNE_FLOOR, } from './buildAnalysisTypes.js';
...
import { getPathAlignmentScoreForTechnique, getPathAlignmentStrengthForTechnique, getTechniqueTaxonomyProfile, } from './techniqueTaxonomy.js';
function createEmptyFamilyCoverage() {
    return Object.fromEntries(TECHNIQUE_FAMILY_ORDER.map((family) => [family, 0]));
}
function createEmptySupportCoverage() {
    return Object.fromEntries(TECHNIQUE_SUPPORT_FLAG_ORDER.map((flag) => [flag, 0]));
}
function buildTechniqueEntry(snapshot, techId, slotType) {
    ...
    const rankFloor = Math.min(progression.rankCap, slotType === 'ultimate' ? ULTIMATE_MIN_RANK_FLOOR : ACTIVE_PASSIVE_MIN_RANK_FLOOR);
    let runeFloor = 0;
    let runeFloorMet = true;
    if (progression.runeSockets > 0) {
        runeFloor = Math.min(progression.runeSockets, slotType === 'ultimate' ? ULTIMATE_MIN_RUNE_FLOOR : ACTIVE_PASSIVE_MIN_RUNE_FLOOR);
...
function makeGap(code, severity, reason) {
    return { code, severity, reason };
}
export function buildBuildAnalysisInput(snapshot) {
    try {
...
export function analyzeBuildFromInput(input) {
    ...
    const pathAlignmentScore = input.snapshot.path === null || equippedTechniques.length === 0
        ? 0
        : Math.round((equippedTechniques.reduce((acc, tech) => acc + tech.pathFitScore, 0) / (equippedTechniques.length * 2)) * 100);
    ...
    const hasSurvivalTool = (supportCoverage.survival ?? 0) > 0 ||
        (familyCoverage.guard ?? 0) > 0 ||
        (familyCoverage.heal ?? 0) > 0 ||
        (familyCoverage.cleanse ?? 0) > 0;
    const hasSetupTool = (familyCoverage.setup ?? 0) > 0 ||
    ...
    if (!hasSurvivalTool && input.loadoutSnapshot.emptyUnlockedCount === 0) {
        gaps.push(makeGap('missing_survival_tool', input.snapshot.path === 'earth' ? 'high' : 'medium', 'The current build lacks a real survival tool.'));
    }
    if (!hasSetupTool && input.loadoutSnapshot.emptyUnlockedCount === 0) {
        gaps.push(makeGap('missing_setup_tool', input.snapshot.path === 'earth' ? 'low' : 'medium', 'The current build lacks setup/control support.'));
    }
    if (masteryFailures.length > 0) {
        gaps.push(makeGap('low_mastery', masteryFailures.some((tech) => tech.slotType === 'ultimate') || masteryFailures.length >= 2 ? 'high' : 'medium', 'One or more equipped techniques are below the semester mastery floor.'));
    }
    if (rankFailures.length > 0) {
        gaps.push(makeGap('low_rank', rankFailures.some((tech) => tech.slotType === 'ultimate') || rankFailures.length >= 2 ? 'high' : 'medium', 'One or more equipped techniques are below the semester rank floor.'));
    }
    if (runeFailures.length > 0) {
        gaps.push(makeGap('rune_gap', runeFailures.some((tech) => tech.slotType === 'ultimate') || runeFailures.length >= 2 ? 'medium' : 'low', 'One or more equipped techniques have rune sockets below the semester floor.'));
    }
...
export function analyzeSelectedBuild(snapshot) {
