...
export const DRIFT_OWNER_PACKET = {
...
const ownerFor = (category) => DRIFT_OWNER_PACKET[category];
const issue = (input) => ({
    ...input,
    suggestedOwnerPacket: ownerFor(input.category),
});
export const collectProgressionDiagnostics = (contract, inputs) => {
    const issues = [];
    ...
    const legacyGateMentions = runtimeEntries.filter(([, text]) => ['foundation_pill', 'core_catalyst', 'core_stabilizer', 'soul_condensate'].some((legacy) => text.includes(legacy)));
    ...
    inputs.authoredContent.items.forEach((itemId) => {
        if (itemId.startsWith('gate_') && !canonicalGateItems.has(itemId)) {
            issues.push(issue({
    ...
    if (contract.contentCap.realmId !== 'spirit_severing') {
        issues.push(issue({
