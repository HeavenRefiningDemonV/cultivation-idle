...
export const DEFERRED_PRESTIGE_REFUND_COSTS = {
...
const PRESTIGE_UPGRADE_COSTS_BY_ID = {
...
const getNumericEffect = (effect, key) => {
    if (!isRecord(effect))
        return null;
    return typeof effect[key] === 'number' ? effect[key] : null;
};
export const getPrestigeRuntimeConsumers = (upgrade) => {
...
export const getPrestigeNodeRuntimeStatus = (nodeId, upgrade) => {
    if (DEFERRED_PRESTIGE_NODE_IDS.includes(nodeId))
        return 'deferred';
    if (nodeId.startsWith('future_') || nodeId.startsWith('deferred_'))
        return 'deferred';
    if (upgrade) {
        return getPrestigeRuntimeConsumers(upgrade).length > 0 ? 'visible_live' : 'hidden_unsupported';
    }
    if (VISIBLE_LIVE_PRESTIGE_NODE_IDS.includes(nodeId))
        return 'visible_live';
    if (LIVE_PREFIXES.some((prefix) => nodeId.startsWith(prefix)))
        return 'hidden_unsupported';
    return 'unknown';
};
export const isRefundableHiddenPrestigeNode = (nodeId, upgrade) => {
    const status = getPrestigeNodeRuntimeStatus(nodeId, upgrade);
    return status === 'hidden_unsupported' || status === 'deferred';
};
export const getPrestigeRefundAmount = (nodeId, purchasedLevels, upgrade) => {
    if (purchasedLevels <= 0)
        return 0;
    const authoredCosts = upgrade?.costs?.filter((cost) => typeof cost === 'number' && Number.isFinite(cost) && cost >= 0);
...
export const createPrestigeClassificationHooks = () => ({
    classifyNode: (nodeId) => {
        if (DEFERRED_PRESTIGE_NODE_IDS.includes(nodeId))
            return 'deferred';
        if (LIVE_PREFIXES.some((prefix) => nodeId.startsWith(prefix)))
