...
export function resolveOfflineCultivationEfficiency(input) {
    ...
    return Math.min(OFFLINE_POLICY.maxEfficiency, Math.max(0, OFFLINE_POLICY.baseEfficiency + additive + heartLawBonus));
}
export function formatOfflineDuration(seconds) {
    if (seconds <= 0) {
        return '0 seconds';
    }
