...
import { advancePromptStates, applyYieldBonuses, completePrompt as completeAssistedPrompt, instantiatePrompts, } from '../systems/crafting/assistedPrompts.js';
...
const sanitizePromptState = (raw) => {
    ...
    if (typeof record.id !== 'string' || typeof record.type !== 'string')
        return null;
    if (typeof record.dueAtMs !== 'number' || typeof record.expiresAtMs !== 'number')
        return null;
    const status = sanitizePromptStatus(record.status);
    if (!status)
        return null;
    const completedAtMs = record.completedAtMs === null || typeof record.completedAtMs === 'number' ? record.completedAtMs : null;
...
const sanitizeForgeStepResult = (raw) => {
    if (!raw || typeof raw !== 'object')
        return null;
    const record = raw;
    ...
    switch (record.type) {
        ...
        case 'QUENCH':
            return {
                stepId: record.stepId,
                type: 'QUENCH',
                medium: record.medium === 'water' || record.medium === 'oil' || record.medium === 'brine'
                    ? record.medium
                    : 'water',
...
const sanitizeForgeStepResults = (raw) => {
...
const isCraftMode = (value) => value === 'idle' || value === 'assisted' || value === 'handsOn';
const isSupportedMode = (value) => value === 'assisted' || value === 'handsOn';
...
const computeHandsOnResult = (active, recipe, masteryBefore, now) => {
    ...
    const totalWeighted = (heatScore / 100) * HANDS_ON_WEIGHTS.heat +
        (stabilityScore / 100) * HANDS_ON_WEIGHTS.stability +
        (orderScore / 100) * HANDS_ON_WEIGHTS.order +
        (qteScore / 100) * HANDS_ON_WEIGHTS.qte;
    let grade = 'crude';
    if (totalWeighted >= 0.9 && impurities === 0)
        grade = 'perfect';
    else if (totalWeighted >= 0.75)
        grade = 'high';
    else if (totalWeighted >= 0.55)
        grade = 'mid';
    else if (totalWeighted >= 0.35)
        grade = 'low';
    ...
    const byproductsGranted = byproductBase > 0 ? [{ itemId: 'mat_aura_residue', qty: byproductBase * active.qty }] : [];
    const baselineTimeSec = typeof active.script.baselineTimeSec === 'number'
...
const computeBaselineResolveAt = (session) => {
    const baselineDurationMs = Math.max(0, Math.floor((session.script.baselineTimeSec ?? (session.endsAt - session.startedAt) / 1000) * 1000));
    return session.createdAt + baselineDurationMs;
};
const buildCurrencyCosts = (rawCosts, qty) => {
    const totals = {};
...
const buildItemCosts = (items, qty) => (items ?? [])
...
const sanitizePayment = (payment) => {
    ...
    if (payment?.currencies) {
        ...
        Object.keys(payment.currencies).forEach((key) => {
...
const isValidCraftStation = (value) => value === 'alchemy' || value === 'forge' || value === 'talisman';
const sanitizeActiveSession = (raw) => {
    if (!raw || typeof raw !== 'object')
        return null;
    const record = raw;
    if (typeof record.sessionId !== 'string')
        return null;
    if (!isValidCraftStation(record.station))
        return null;
    if (!isSupportedMode(record.mode))
        return null;
    if (typeof record.sourceId !== 'string')
        return null;
    if (typeof record.qty !== 'number' || !Number.isFinite(record.qty))
        return null;
    if (typeof record.createdAt !== 'number' || typeof record.seed !== 'number')
        return null;
    ...
    if (!Array.isArray(script.steps))
        return null;
    if (!script.steps.every((step) => step && typeof step.id === 'string' && typeof step.type === 'string'))
        return null;
    const payment = sanitizePayment(record.payment);
    const cursor = typeof record.cursor === 'object' &&
        record.cursor !== null &&
        typeof record.cursor.stepIndex === 'number'
        ? {
            stepIndex: record.cursor.stepIndex,
            stepStartedAt: record.cursor.stepStartedAt === undefined || typeof record.cursor.stepStartedAt === 'number'
                ? record.cursor.stepStartedAt
                : undefined,
            stepEndsAt: record.cursor.stepEndsAt === undefined || typeof record.cursor.stepEndsAt === 'number'
                ? record.cursor.stepEndsAt
                : undefined,
            heatSetting: record.cursor.heatSetting === undefined || typeof record.cursor.heatSetting === 'number'
                ? record.cursor.heatSetting
                : undefined,
            impurities: record.cursor.impurities === undefined || typeof record.cursor.impurities === 'number'
                ? record.cursor.impurities
                : undefined,
            scoreParts: record.cursor.scoreParts && typeof record.cursor.scoreParts === 'object'
                ? { ...record.cursor.scoreParts }
                : undefined,
            orderMistakes: record.cursor.orderMistakes === undefined || typeof record.cursor.orderMistakes === 'number'
                ? record.cursor.orderMistakes
                : undefined,
            backgroundResolveAt: record.cursor.backgroundResolveAt === undefined || record.cursor.backgroundResolveAt === null
                ? record.cursor.backgroundResolveAt
                : typeof record.cursor.backgroundResolveAt === 'number'
                    ? record.cursor.backgroundResolveAt
                    : null,
            backgroundReason: record.cursor.backgroundReason === undefined || record.cursor.backgroundReason === null
                ? record.cursor.backgroundReason
                : record.cursor.backgroundReason === 'closed' ||
                    record.cursor.backgroundReason === 'navigated' ||
                    record.cursor.backgroundReason === 'crashed'
                    ? record.cursor.backgroundReason
                    : null,
            forgeStepResults: sanitizeForgeStepResults(record.cursor.forgeStepResults),
        }
        : { stepIndex: 0 };
...
export const createDefaultCraftSessionState = () => ({
    modeByStation: { ...defaultModes },
    activeSession: null,
});
export const useCraftSessionStore = create()(immer((set, get) => {
    const resolveSessionToBaseline = (active, now) => {
        ...
        if (active.station === 'alchemy') {
            ...
            const baseItems = buildAlchemyOutputs(recipe.outputs, active.qty, getIdleYieldMultiplierForMastery(mastery));
    ...
    return {
        ...
        setMode: (station, mode) => {
            if (!isCraftMode(mode) || !isValidCraftStation(station))
                return;
            set((state) => {
                state.modeByStation[station] = mode;
            });
            GameEvents.emit({ type: 'crafting/mode_selected', payload: { station, mode } });
        },
        startSession: (args) => {
            if (get().activeSession)
                return { ok: false, reason: 'active_session' };
            if (!isSupportedMode(args.mode))
                return { ok: false, reason: 'invalid_mode' };
            if (!isValidCraftStation(args.station))
            ...
            if (args.station === 'alchemy') {
                ...
                const currencies = buildCurrencyCosts(recipe.costs, qty);
                if (Object.keys(currencies).length > 0) {
                    payment.currencies = currencies;
                }
                script = buildAlchemyScript(args.sourceId, seed);
                const durationMs = calculateAlchemyDurationMs(recipe.timeSec ?? recipe.craftTimeSec, qty);
                startedAt = createdAt;
                endsAt = startedAt + durationMs;
                const promptDefs = Array.isArray(recipe.assistedPrompts)
                    ? recipe.assistedPrompts
                    : [];
                ...
                const currencies = buildCurrencyCosts({ gold: blueprint.costs.gold, spiritStones: blueprint.costs.spiritStones }, qty);
                if (Object.keys(currencies).length > 0) {
                    payment.currencies = currencies;
                }
                ...
                startedAt = createdAt;
                endsAt = startedAt + durationMs;
                const promptDefs = Array.isArray(rawBlueprint?.assistedPrompts)
                    ? rawBlueprint.assistedPrompts
                    : [];
                if (args.mode === 'assisted') {
                    prompts = instantiatePrompts(promptDefs, startedAt, endsAt, seed);
                }
                ...
                for (const cost of itemCosts) {
                    ...
                    if (!success) {
                        if (Object.keys(spentCurrencies).length > 0) {
                            Object.keys(spentCurrencies).forEach((key) => {
            ...
            GameEvents.emit({
                ...
                payload: { station: session.station, mode: session.mode, sourceId: session.sourceId },
            });
            if (session.station === 'alchemy') {
                GameEvents.emit({ type: 'alchemy/flame_ignite', payload: { recipeId: session.sourceId } });
            }
            if (session.station === 'forge') {
                GameEvents.emit({ type: 'forge/furnace_ignite', payload: { blueprintId: session.sourceId } });
            }
        ...
        updateActiveSessionPrompts: (now = Date.now()) => {
            ...
            const prevPrompts = active.prompts;
            const prevStatus = new Map(prevPrompts.map((prompt) => [prompt.id, prompt.status]));
            const nextPrompts = advancePromptStates(prevPrompts, now);
            set((state) => {
                if (state.activeSession) {
            ...
            nextPrompts.forEach((prompt) => {
                const previous = prevStatus.get(prompt.id);
                if (previous === prompt.status)
                    return;
                if (prompt.status === 'AVAILABLE') {
                    GameEvents.emit({
                        type: 'crafting/assist_prompt_available',
                        payload: { station: active.station, promptId: prompt.id, promptType: prompt.type },
                    });
                }
                else if (prompt.status === 'MISSED') {
                    GameEvents.emit({
        ...
        completePrompt: (promptId, now = Date.now()) => {
            ...
            if (!active)
                return { ok: false, reason: 'no_session' };
            if (active.mode !== 'assisted')
                return { ok: false, reason: 'wrong_mode' };
            const prompt = active.prompts?.find((entry) => entry.id === promptId);
        ...
        recordForgeStepResult: (result) => {
            ...
            set((state) => {
                ...
                const filtered = existing.filter((entry) => !(entry.stepId === sanitized.stepId && entry.type === sanitized.type));
        ...
        markBackgroundResolving: (reason) => {
            ...
            GameEvents.emit({
                ...
                payload: { station: active.station, mode: active.mode, sourceId: active.sourceId, reason },
            });
        },
        tick: (now = Date.now()) => {
            const active = get().activeSession;
        ...
        completeHandsOnSession: (now = Date.now()) => {
            ...
            if (active.station === 'alchemy') {
                ...
                GameEvents.emit({ type: 'alchemy/result', payload: { grade: result.grade } });
                if (result.byproductsGranted.length > 0) {
                    const totalByproducts = result.byproductsGranted.reduce((sum, entry) => sum + entry.qty, 0);
                    GameEvents.emit({ type: 'alchemy/byproduct_gain', payload: { qty: totalByproducts } });
                }
                GameEvents.emit({
        ...
        hydrate: (slice) => {
            ...
            if (slice?.modeByStation && typeof slice.modeByStation === 'object') {
                Object.keys(slice.modeByStation).forEach((station) => {
            ...
            if (activeSession?.mode === 'assisted') {
                ...
                if (hydratedSession.station === 'alchemy') {
                    ...
                    const promptDefs = Array.isArray(recipe?.assistedPrompts)
                        ? recipe.assistedPrompts
                        : [];
                    const durationMs = calculateAlchemyDurationMs(recipe?.timeSec ?? recipe?.craftTimeSec, hydratedSession.qty);
                    const endsAt = hydratedSession.endsAt || hydratedSession.startedAt + durationMs;
                    const prompts = hydratedSession.prompts && hydratedSession.prompts.length > 0
