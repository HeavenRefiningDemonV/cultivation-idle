...
export const formatGameEventSummary = (event) => {
    switch (event.type) {
        ...
        case 'trials/attempt_resolved': {
            ...
            if (outcome === 'defeated')
                return `Trial defeat: Gate ${gateIndex} / boss ${Math.round((bossHpPctRemaining ?? 0) * 100)}%`;
...
export const useTelemetryStore = create()(immer((set) => ({
    events: [],
    ...
    addBalanceEvent: (entry) => {
        set((state) => {
            if (!state.balanceCaptureEnabled)
                return;
    ...
    setBalanceCaptureEnabled: (enabled) => {
        set((state) => {
