...
import { createCapReachedScenario, createFreshLifeScenario, createLegacyAliasScenario, loadProgressionContract, } from '../helpers/progression/index.js';
...
const loadRawContent = async () => {
    const entries = await Promise.all(Object.entries(FILES).map(async ([key, fileName]) => [key, await readJson(path.join(CONTENT_DIR, fileName))]));
