...
test('balance telemetry captures major hook families from runtime events', () => {
  ...
  GameEvents.emit({ type: 'rewards/granted', payload: { timestamp: now, reason: 'Outskirts Victory (Mob)', summary: 'ok', bundle: { currencies: { gold: '30' } }, result: { appliedCurrencies: { gold: '30' }, appliedItems: [], droppedItems: [], appliedTechniqueFragments: [], appliedManuals: [], appliedComprehension: null, skippedRewards: [] } } });
