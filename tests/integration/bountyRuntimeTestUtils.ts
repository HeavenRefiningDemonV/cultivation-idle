...
export async function loadValidatedContent(): Promise<LoadedValidatedContent> {
...
export function resetBountyRuntimeStores() {
...
export async function primeBountyRuntimeStores() {
  const { validated, citiesSorted } = await loadValidatedContent();
  ...
  useContentStore.setState({
    ...
    maps: {
      ...
      prestigeUpgradesById: Object.fromEntries(
        validated.prestige_store.upgrades.map((upgrade) => [upgrade.id, upgrade]),
      ),
