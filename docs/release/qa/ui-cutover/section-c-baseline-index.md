...
| `life-start-breath-focus` | Life Start Breath Focus (Step 3) | ritual modal | `/?uiAudit=section-c&surface=life-start-breath-focus&fx=high` | CAPTURE PENDING — MANUAL | state-gated live flow (forced pin for capture) | Existing breath mode cards and finish actions inside current life-start shell. | C.4 | Surface is live in the wizard flow; harness still pins Step 3 for deterministic capture. |
