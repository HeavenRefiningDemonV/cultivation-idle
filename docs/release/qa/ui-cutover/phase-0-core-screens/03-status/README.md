# 03-status evidence status

- Screen: Status
- Signoff state: `DEFERRED`
- Evidence status: proof gap (no complete legal `01-06` image set committed yet)
- Existing packet evidence reference: `docs/release/qa/ui-cutover/phase-0-p0-06-status/README.md`

## Approved capture pipeline
1. Run `npm run release:phase0-core-capture`.
2. The harness route for this surface is `/?uiAudit=phase-0&surface=status&fx=<high|low|reduced>&slot=<slot>`.
