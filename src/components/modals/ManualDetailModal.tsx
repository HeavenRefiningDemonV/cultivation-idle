...
export type ManualPurchaseResult = StoreManualPurchaseResult & { studied?: boolean };
...
const rarityLabel = (value: ManualRarity) => value.charAt(0).toUpperCase() + value.slice(1);
const gradeLabel = (value: ManualGrade) => value.charAt(0).toUpperCase() + value.slice(1);
...
export function ManualDetailModal({
  ...
  const sections = useMemo(() => {
    ...
    const cooldown = technique?.cooldownSec != null ? `${technique.cooldownSec}s` : '—';
    const resourceLine =
      ...
      },
      {
        ...
        content: (
          <div className="pavilionDetailPanel">
            ...
            {formatEffectBlock('Mastery 75 Bonus', technique?.secondaryAtMastery75)}
          </div>
        ),
      },
      {
        ...
        content: (
          <div className="pavilionDetailPanel">
            ...
            {manual.slot.sold && <div className={'pavilionDetailLine pavilionCardSold'}>Sold out.</div>}
  ...
  useEffect(() => {
    ...
    const sectionElements = Array.from(scrollContainer.querySelectorAll<HTMLElement>('[data-section-id]'));
  ...
  return createPortal(
    <div className={'pavilionDetailOverlay'} role="presentation" onMouseDown={handleClose}>
      <div
      ...
      >
        <div className="pavilionDetailHeader">
          <div className="pavilionDetailTitleBlock">
            ...
            <div className="pavilionDetailSubtitle">{manual.slot.techniqueId}</div>
          </div>
        ...
        <div className="pavilionDetailBody">
          <div className="pavilionDetailBodyLayout">
            ...
            <div className="pavilionDetailScroll" ref={scrollContainerRef} onScroll={handleScroll}>
              <div
              ...
              <div className="pavilionDetailContent">
                {sections.map((section) => (
                  ...
                  >
                    <h3 className="pavilionDetailSectionTitle" id={`${titleId}-${section.id}`}>
                      {section.title}
                    </h3>
                    {section.content}
                  </section>
          ...
          {hasBackToTop && (
            <button className="pavilionDetailBackToTop" type="button" onClick={handleBackToTop}>
              Back to top
            </button>
          )}
        </div>
...
function renderPurchaseResult(
  ...
  return (
    <div className={'pavilionPurchaseResult pavilionPurchaseResult--duplicate'}>
      ...
      <div>
        +{result.fragmentsGained} Technique Fragments ({rarityLabel(result.rarity ?? 'common')})
      </div>
