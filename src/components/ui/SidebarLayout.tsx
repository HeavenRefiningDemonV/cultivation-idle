...
export function SidebarLayout({ activeTab, onSelectTab, children }: SidebarLayoutProps) {
  ...
  return (
    <div className="flex min-h-screen pt-20 text-ink-paper">
        ...
        <div className="p-4 space-y-4">
          <div className="bg-ink-darkest/70 border border-ink-medium rounded-lg p-3">
            ...
            <div className="text-sm font-semibold text-gold-accent">
              {realm?.name}
              {realm ? <span className="ml-1 text-ink-light">• Substage {realm.substage + 1}</span> : null}
            </div>
          </div>
      ...
      <div className="flex-1 min-h-[calc(100vh-5rem)] overflow-y-auto p-4">{children}</div>
    </div>
  );
}
