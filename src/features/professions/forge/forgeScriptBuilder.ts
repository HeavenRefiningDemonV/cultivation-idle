...
export type BuildDefaultForgeScriptArgs = {
  blueprintId: string;
  specialStep?: SpecialStepType;
  runeFamily?: string;
  formationId?: string;
};
...
const RING_QTE_TYPES = new Set<ForgeStepDef['type']>(['HAMMER_PATTERN', 'ENGRAVE_RUNE', 'LAY_FORMATION']);

const resolvePatternId = (blueprintId: string, stepType: ForgeStepDef['type'], index: number): string =>
  `${blueprintId}:${stepType}:${index}`;

const applyPatternIds = (steps: ForgeStepDef[], blueprintId: string): ForgeStepDef[] =>
  steps.map((step, index) => {
    if (!RING_QTE_TYPES.has(step.type)) return step;
...
const resolveSpecialStep = (blueprint: NormalizedForgeBlueprint): BuildDefaultForgeScriptArgs => {
  if (blueprint.type === 'service') {
    if (blueprint.service === 'temper') return { blueprintId: blueprint.id, specialStep: 'TEMPER' };
    if (blueprint.service === 'refine') return { blueprintId: blueprint.id, specialStep: 'TEMPER' };
  }
...
export function buildDefaultForgeScript(args: BuildDefaultForgeScriptArgs): ForgeStepDef[] {
...
export function resolveForgeStepScript(blueprint: NormalizedForgeBlueprint): ForgeStepDef[] {
