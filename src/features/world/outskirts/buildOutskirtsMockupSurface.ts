...
import {
  ...
  OUTSKIRTS_LIVE_SCENIC_FALLBACK_SRC,
...
function resolveScenicImageSrc(backgroundKey: string, sourceMode: OutskirtsMockupRuntimeSnapshot['sourceMode']): string | null {
  if (sourceMode === 'fixture') return OUTSKIRTS_APPROVED_SCENIC_MOCKUP_SRC;
  if (backgroundKey.startsWith('placeholder/')) return OUTSKIRTS_LIVE_SCENIC_FALLBACK_SRC;
  return OUTSKIRTS_LIVE_SCENIC_FALLBACK_SRC;
}
...
export function buildOutskirtsMockupSurface(snapshot: OutskirtsMockupRuntimeSnapshot): OutskirtsExactSurfaceV2 {
  ...
  return {
    ...
    scenicStage: {
      ...
      scenicImageSrc: resolveScenicImageSrc(snapshot.scenicBackgroundKey, snapshot.sourceMode),
      reviewFixtureImageSrc: OUTSKIRTS_APPROVED_SCENIC_MOCKUP_SRC,
      liveFallbackImageSrc: OUTSKIRTS_LIVE_SCENIC_FALLBACK_SRC,
