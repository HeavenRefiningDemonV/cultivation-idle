...
export function createRuinsExactMockupFixture(overrides: Partial<RuinsExactSurfaceV1> = {}): RuinsExactSurfaceV1 {
  const surface: RuinsExactSurfaceV1 = {
    ...
    page: { title: 'Ruins' },
    topRibbon: { ariaLabel: 'Ruins relief route', decorative: true, activeNodeId: 'n9', nodes: Array.from({ length: 15 }, (_, i) => ({ id: `n${i + 1}`, label: `${i + 1}`, state: i < 8 ? 'completed' : i === 8 ? 'current' : 'future', variant: i === 8 ? 'active' : 'muted' })) },
    ...
    shell,
    debug: { regionOrder: RUINS_EXACT_REGION_ORDER, missingDataFallbacks: [], placeholderAssetKeysInUse: [], liveSourceNotes: ['Central scenic art uses the reserved Hollow Log Den plate slot until the final approved plate is bound.'], fixtureLockedValues: ['Rare Pity 1 / 6', 'Continue Exploration'] },
  };
...
export function buildRuinsExactSurfaceFromStores(cityId?: string, options: BuildRuinsExactSurfaceOptions = {}): RuinsExactSurfaceV1 {
  ...
  return {
    ...
    debug: { regionOrder: RUINS_EXACT_REGION_ORDER, missingDataFallbacks: missing, placeholderAssetKeysInUse: [], liveSourceNotes: [...notes, 'Central scenic art uses the reserved Hollow Log Den plate slot until the final approved plate is bound.'], fixtureLockedValues: ['Rare Pity 1 / 6 (fixture only)'] },
  };
}
