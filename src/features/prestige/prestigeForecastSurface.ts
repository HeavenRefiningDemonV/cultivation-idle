...
import { getPrestigeResetContractSurface, type PrestigeResetBucketLine } from '../../services/prestige/PrestigeResetContract.js';
...
export type PrestigeReclaimForecastLine = {
  id: string;
  label: string;
  previous: string;
  expectedNext: string;
...
export type PrestigeRecommendedPurchaseLine = {
  id: string;
  upgradeId: string;
  label: string;
  cost: number;
  currentLevel: number;
  nextLevel: number;
  affordance: 'buy_now' | 'save_for_next' | 'owned' | 'deferred' | 'hidden_unsupported';
  reason: string;
  runtimeHonesty: PrestigeEffectRuntimeHonesty;
};

export type PrestigePostResetObjective = {
  id: string;
  label: string;
  detail: string;
  route: {
    kind: 'tab' | 'world_module' | 'life_start' | 'status' | 'none';
    tabId?: string;
    moduleKey?: string;
    anchor?: string;
  } | null;
...
export type PrestigeForecastSurfaceV2 = {
  version: 2;
  advisorState: PrestigeForecastAdvisorState;
  advisorLabel: string;
  ...
  ap: {
    ...
    afterRitual: number;
    lifetimeAP: number;
    breakdownRows: { key: string; label: string; value: number; hint?: string }[];
  };
  resetBuckets: PrestigeResetBucketLine[];
  carryBuckets: PrestigeResetBucketLine[];
  rebuiltBuckets: PrestigeResetBucketLine[];
  hybridBuckets: PrestigeResetBucketLine[];
  retainedEffects: PrestigeRetainedEffectLine[];
  recommendedPurchases: PrestigeRecommendedPurchaseLine[];
  reclaimForecast: PrestigeReclaimForecastLine[];
  postResetObjective: PrestigePostResetObjective;
  warnings: string[];
  debugNotes: string[];
};
...
const mapAdvisorState = (atContentCap: boolean, label: string): PrestigeForecastAdvisorState => {
  if (label === 'Too Early') return 'too_early';
  if (atContentCap) return 'chapter_exhausted';
  if (label === 'Recommended') return 'recommended';
  return 'viable';
};

const buildAdvisorCopy = (state: PrestigeForecastAdvisorState): { label: string; detail: string } => {
  if (state === 'too_early') {
    return {
      label: 'Too Early',
...
const getNextCost = (costs: readonly number[] | undefined, currentLevel: number): number | null => {
  const cost = costs?.[currentLevel];
  return typeof cost === 'number' && Number.isFinite(cost) ? cost : null;
};

const runtimeHonestyFromStatus = (status: string): PrestigeRecommendedPurchaseLine['runtimeHonesty'] => {
  if (status === 'visible_live') return 'live';
  if (status === 'deferred') return 'deferred';
  if (status === 'hidden_unsupported') return 'unsupported_hidden';
  return 'unknown_blocked';
};

const buildRecommendedPurchases = (apBudget?: number): PrestigeRecommendedPurchaseLine[] => {
  const content = useContentStore.getState().raw;
  if (!content?.prestige_store) return [];
  ...
  return plan.orderedPlan.slice(0, 3).flatMap((candidate) => {
    ...
    if (!node || node.status !== 'visible_live') return [];
    ...
    return [{
      ...
      affordance: cost <= effectiveApBudget ? 'buy_now' as const : 'save_for_next' as const,
...
const buildRetainedEffects = (): PrestigeRetainedEffectLine[] => {
  const prestige = usePrestigeStore.getState();
  const content = useContentStore.getState().raw;
  const rows: PrestigeRetainedEffectLine[] = [];
  ...
  const heartLawNode = content ? getPrestigeRuntimeCatalog(content).visibleLiveNodeIds.find((id) =>
    id.startsWith('ap_unlock_heartlaw') && (prestige.purchasesById[id] ?? 0) > 0,
  ) : null;
  if (heartLawNode) {
    rows.push({
...
const estimateReclaimSpeedup = (): number => {
  const prestige = usePrestigeStore.getState();
...
const buildReclaimForecast = (): PrestigeReclaimForecastLine[] => {
  const prestige = usePrestigeStore.getState();
...
export const buildPostResetObjectivePreview = (): PrestigePostResetObjective => {
  const game = useGameStore.getState();
  const prestige = usePrestigeStore.getState();
...
export function buildPrestigeForecastSurfaceV2(): PrestigeForecastSurfaceV2 {
  const prestige = usePrestigeStore.getState();
  const content = useContentStore.getState().raw;
