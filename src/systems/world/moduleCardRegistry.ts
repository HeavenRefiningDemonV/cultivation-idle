...
const DEFINITION_LIST: readonly WorldModuleCardDefinition[] = [
  ...
  },
  {
    ...
    label: getWorldModuleLabel('trainingHall'),
    group: 'preparation',
    sortOrder: 5,
    roleTag: 'Path Foundation',
    bestUsedWhen: 'Best used when path foundation, trained stat caps, or regimen mastery is the current gate gap.',
    defaultOutputs: [{ key: 'stat_ratings', label: 'Stat Ratings' }, { key: 'regimen_mastery', label: 'Regimen Mastery' }],
    ctaLabel: 'Open Training Hall',
    allowedChipKinds: ['recommended_now', 'useful_soon', 'build_fix'],
    allowStrongRecommendation: true,
    allowSecondaryRecommendation: true,
  },
