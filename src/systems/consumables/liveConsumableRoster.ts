...
export type ConsumableAvailability = 'shop' | 'brew' | 'shop_and_brew';
export type LiveConsumableStatus = 'live' | 'hidden' | 'deferred';

export interface LiveConsumableRosterEntry {
  itemId: string;
  domain: LiveConsumableDomain;
  family: CultivationConsumableFamily | 'healing' | 'defense' | 'evasion' | 'ward' | 'utility';
  status: LiveConsumableStatus;
  availability: ConsumableAvailability | 'hidden';
  usage: 'combat_only' | 'combat_or_world' | 'cultivate_only';
}

const CANONICAL_ROSTER: Record<string, Omit<LiveConsumableRosterEntry, 'status' | 'availability' | 'usage'>> = {
...
export function buildLiveConsumableRoster(content: Pick<ValidatedContent, 'items' | 'alchemy_recipes' | 'forge_blueprints' | 'apothecary_shops'>): LiveConsumableRosterEntry[] {
  ...
  return Object.values(CANONICAL_ROSTER).flatMap((entry) => {
    ...
    const status: LiveConsumableStatus = runtimeStatus === 'visible_live' || runtimeStatus === 'visible_live_blocked'
      ? 'live'
    ...
    const availability: LiveConsumableRosterEntry['availability'] = status !== 'live'
      ? 'hidden'
      : isShop && isBrew
        ? 'shop_and_brew'
        : isShop
...
export function getLiveConsumableRoster(content: Pick<ValidatedContent, 'items' | 'alchemy_recipes' | 'forge_blueprints' | 'apothecary_shops'>) {
  return buildLiveConsumableRoster(content);
}

export function getLiveConsumableRosterEntry(
  content: Pick<ValidatedContent, 'items' | 'alchemy_recipes' | 'forge_blueprints' | 'apothecary_shops'>,
  itemId: string,
): LiveConsumableRosterEntry | null {
