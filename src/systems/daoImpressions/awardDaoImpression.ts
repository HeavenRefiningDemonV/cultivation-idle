...
import type { DaoImpressionAward, DaoImpressionCandidate } from './types.js';

function buildAwardId(candidate: DaoImpressionCandidate): string {
  return `dao:${candidate.sourceEventKey.replace(/[^a-zA-Z0-9:_-]/g, '_')}`;
}

function defaultRouteHint(sourceKind: DaoImpressionCandidate['sourceKind']): DaoImpressionAward['routeHint'] {
  switch (sourceKind) {
    case 'gate_clear':
...
function memoryLineFor(candidate: DaoImpressionCandidate, definition: NonNullable<ReturnType<typeof getDaoImpressionDefinition>>, applied: boolean): string {
  if (!applied) {
...
export function awardDaoImpression(candidate: DaoImpressionCandidate): DaoImpressionAward | null {
  const store = useDaoImpressionStore.getState();
  if (store.hasSourceEvent(candidate.sourceEventKey)) return null;

  const definition = getDaoImpressionDefinition(candidate.impressionId);
  if (!definition || definition.sourceKind !== candidate.sourceKind) return null;
  if (definition.runtimeStatus === 'future_stub_only') return null;

  if (definition.cooldownMs != null && definition.cooldownMs > 0) {
    const latestAward =
      store.latestAwardForImpression(definition.impressionId) ?? store.latestAwardForSourceKind(definition.sourceKind);
    if (latestAward && candidate.createdAt - latestAward.createdAt < definition.cooldownMs) return null;
  }
  ...
  const award: DaoImpressionAward = {
