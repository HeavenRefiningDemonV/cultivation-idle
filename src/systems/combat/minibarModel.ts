export type SafetyTier = 'safe' | 'risky' | 'deadly';
...
export function hpPercent(current: string, max: string): number {
...
export function getNextActionTimerMs(now: number, lastActionAt: number, cooldownMs: number): number {
  const remaining = (lastActionAt ?? 0) + (cooldownMs ?? 0) - (now ?? 0);
  if (!Number.isFinite(remaining)) return 0;
  return Math.max(0, remaining);
}

export function getCooldownProgress(now: number, lastActionAt: number, cooldownMs: number): number {
...
export function computeCombatSafety(input: {
  ...
  enemyCritDmgPct: number;
  enemyIsBoss: boolean;
  enemyAuraDps: number;
  enemyHasEnrageMechanic: boolean;
  enemyEnrageMultiplier?: number;
