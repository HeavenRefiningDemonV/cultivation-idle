import type { ProgressionContract } from '../contract/index.js';
import type { DriftIssue } from './driftTypes.js';

export const renderProgressionContractReport = (contract: ProgressionContract, diagnostics: DriftIssue[]): string => {
  const lines: string[] = [];
  ...
  const grouped = diagnostics.reduce<Record<string, DriftIssue[]>>((acc, issue) => {
