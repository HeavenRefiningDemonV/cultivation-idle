...
export interface SlotProgressionRow {
...
const normalizeBonusSlots = (value: number | undefined): number => {
  if (!Number.isFinite(value)) return 0;
  return Math.max(0, Math.floor(value ?? 0));
};

const createRequirement = (realmIndex: number): SlotUnlockRequirement => {
...
export const LOADOUT_PROGRESSION_ORDER: readonly MajorRealmId[] = Object.freeze([
  'qi_condensation',
...
export const SEMESTER_SLOT_PROGRESSION: readonly SlotProgressionRow[] = Object.freeze([
...
] as const satisfies readonly SlotProgressionRow[]);

export const SEMESTER_SLOT_CAPS: Readonly<{ active: number; passive: number }> = Object.freeze({
  active: ACTIVE_SLOT_LIMIT,
  passive: PASSIVE_SLOT_LIMIT,
});

export function getLoadoutProgressionForRealm(majorRealmId: MajorRealmId): SlotProgressionRow {
  return SEMESTER_SLOT_PROGRESSION.find((row) => row.majorRealmId === majorRealmId)
    ?? SEMESTER_SLOT_PROGRESSION[0];
}

export function getLoadoutProgressionForRealmIndex(realmIndex: number): SlotProgressionRow {
...
export function resolveLoadoutProgressionSnapshot(input?: {
  ...
  const activeRequirements = Array.from({ length: ACTIVE_SLOT_LIMIT }, (_, slotIndex) => {
    if (slotIndex < unlocked.active) return [slotIndex, null] as const;
    return [
      slotIndex,
  ...
  const passiveRequirements = Array.from({ length: PASSIVE_SLOT_LIMIT }, (_, slotIndex) => {
    if (slotIndex < unlocked.passive) return [slotIndex, null] as const;
    return [
      slotIndex,
      findUnlockRequirement({
        slotType: 'passive',
...
export function getSlotUnlockRequirementForProgression(input: {
  ...
  const isValid = (
    (input.slotType === 'active' && slotIndex >= 0 && slotIndex < ACTIVE_SLOT_LIMIT)
  ...
  if (!earliestRequirement) return null;
