...
export interface TechniqueSignalSummary {
  primaryEffectTypes: string[];
  secondaryEffectTypes: string[];
  statKeys: string[];
  statusIds: string[];
}

export interface TechniqueFamilyDerivation {
  families: TechniqueFamily[];
  supportFlags: TechniqueSupportFlag[];
  derivedFrom: string[];
}

export const TECHNIQUE_FAMILY_ORDER: readonly TechniqueFamily[] = Object.freeze([
  'coreDamage',
  'aoe',
  'execute',
  'guard',
...
export const TECHNIQUE_SUPPORT_FLAG_ORDER: readonly TechniqueSupportFlag[] = Object.freeze([
  'survival',
  'tempo',
  'boss',
  'farm',
...
export const RECOGNIZED_PRIMARY_EFFECT_TYPES: readonly string[] = Object.freeze([
  'applyStatus',
  'buff',
  'cleanse',
  'composite',
...
export const RECOGNIZED_SECONDARY_EFFECT_TYPES: readonly string[] = Object.freeze([
  'addBuffOnCast',
  'addDebuffOnCast',
  'addDebuffOnHit',
  'addOnCast',
...
function lowerCaseList(values: readonly string[] | undefined): string[] {
...
function sortAndDedupeStrings(values: Iterable<string>): string[] {
  return Array.from(new Set(values)).sort((a, b) => a.localeCompare(b));
}

function sortFamilies(values: Iterable<TechniqueFamily>): TechniqueFamily[] {
  const seen = new Set(values);
  return TECHNIQUE_FAMILY_ORDER.filter((family) => seen.has(family));
}

function sortSupportFlags(values: Iterable<TechniqueSupportFlag>): TechniqueSupportFlag[] {
  const seen = new Set(values);
  return TECHNIQUE_SUPPORT_FLAG_ORDER.filter((flag) => seen.has(flag));
}

function collectPrimaryEffectTypes(node: unknown, collector: Set<string>): void {
...
function collectStatKeys(node: unknown, collector: Set<string>): void {
  if (Array.isArray(node)) {
...
function collectStatusIds(node: unknown, collector: Set<string>): void {
  if (Array.isArray(node)) {
...
function hasTag(def: TechniqueDef, tag: string): boolean {
  return lowerCaseList(def.tags).includes(tag);
}

function hasAnyTag(def: TechniqueDef, tags: readonly string[]): boolean {
...
function hasSecondaryEffectType(summary: TechniqueSignalSummary, type: string): boolean {
  return summary.secondaryEffectTypes.includes(type);
}

function hasAnyStat(summary: TechniqueSignalSummary, group: ReadonlySet<string>): boolean {
  return summary.statKeys.some((stat) => group.has(stat));
}

function hasAnyStatus(summary: TechniqueSignalSummary, group: ReadonlySet<string>): boolean {
  return summary.statusIds.some((status) => group.has(status));
}

function walkValues(node: unknown, visitor: (record: Record<string, unknown>) => void): void {
  if (Array.isArray(node)) {
...
function hasGuaranteedCrit(def: TechniqueDef): boolean {
  let found = false;
...
function hasLargeDamageMultiplier(def: TechniqueDef): boolean {
  let found = false;
  walkValues(def.effect, (record) => {
    const type = asString(record.type);
...
function hasBurstCritInjection(def: TechniqueDef): boolean {
...
function hasIgnoreDefSignal(summary: TechniqueSignalSummary): boolean {
  return summary.statKeys.includes('ignoredefpct');
}

function hasSelfBuffAndStat(def: TechniqueDef, statKeys: readonly string[]): boolean {
...
function deriveFamiliesWithTrace(def: TechniqueDef): { families: TechniqueFamily[]; fallbackMarkers: string[] } {
  ...
  const isClearlyDamage =
    hasPrimaryEffectType(summary, 'damage') ||
  ...
  if (
    families.has('coreDamage') && (
      (def.type === 'ultimate' && isClearlyDamage) ||
      hasTag(def, 'burst') ||
  ...
  if (
    hasPrimaryEffectType(summary, 'heal') ||
  ...
  if (
    hasAnyStat(summary, CONTROL_ENEMY_STAT_KEYS) ||
  ...
  if (
    hasPrimaryEffectType(summary, 'cleanse') ||
...
export function buildTechniqueSignalSummary(def: TechniqueDef): TechniqueSignalSummary {
  const primaryEffectTypes = new Set<string>();
  const secondaryEffectTypes = new Set<string>();
...
export function deriveTechniqueFamilies(def: TechniqueDef): TechniqueFamily[] {
  return deriveFamiliesWithTrace(def).families;
}

export function deriveTechniqueSupportFlags(
  def: TechniqueDef,
  ...
  const summary = buildTechniqueSignalSummary(def);
  const flags = new Set<TechniqueSupportFlag>();
  ...
  if (
    Array.from(TEMPO_TAGS).some((tag) => lowerTags.has(tag)) ||
...
export function buildTechniqueFamilyDerivation(def: TechniqueDef): TechniqueFamilyDerivation {
  const summary = buildTechniqueSignalSummary(def);
