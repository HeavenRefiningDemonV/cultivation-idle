...
import type { CombatEncounterType, CombatPostureRating, MedicinePouchFit } from './combatPostureTypes.js';
...
function isCombatConsumableForSlot(
  slot: MedicinePouchSlotState | null | undefined,
...
function getUtilityOrSpecialtyStatus(
  slot: MedicinePouchSlotState | null | undefined,
  slotKey: 'utility' | 'specialty',
) {
...
export function evaluateMedicinePouchFit(input: {
  ...
  const healingCorrect =
    Boolean(healingSlot?.enabled) &&
  ...
  const healingRiskyPresent =
    Boolean(healingSlot?.enabled) && healingHasCombatConsumable && !healingCorrect;
  ...
  const hasEnabledCombatUtilityOrSpecialty =
    (utilityStatus.enabled && utilityStatus.carriesCombatConsumable) ||
    (specialtyStatus.enabled && specialtyStatus.carriesCombatConsumable);
  const allEnabledCombatUtilityOrSpecialtyManualOnly =
    hasEnabledCombatUtilityOrSpecialty &&
    !hasUsefulUtilityOrSpecialty &&
    [utilityStatus, specialtyStatus].every(
      (status) => !status.enabled || !status.carriesCombatConsumable || status.manualOnly,
    );
  const hasEnabledEquippedSpecialtyCombatSlot =
    specialtyStatus.enabled && specialtyStatus.carriesCombatConsumable;
  const specialtyBossTimed =
    specialtyStatus.useful &&
    !!specialtySlot &&
  ...
  if (healingRiskyPresent) {
    pushWarning(
      warnings,
      'Healing pouch should auto-trigger on low HP between 20% and 45%.',
    );
  }
  ...
  if (
    ...
    pushWarning(
      warnings,
      'Utility or specialty pouch should auto-trigger instead of staying manual-only in serious fights.',
    );
  }
  ...
  if (
    ...
    pushWarning(
      warnings,
      'Trial posture wants a boss-timed specialty pouch if one is equipped.',
    );
  }
