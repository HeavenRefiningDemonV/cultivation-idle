...
export const BUILD_ARCHETYPE_ORDER: readonly string[] = Object.freeze([
...
const freezeArchetype = (profile: BuildArchetypeProfile): BuildArchetypeProfile => {
...
export const SEMESTER_BUILD_ARCHETYPES: readonly BuildArchetypeProfile[] = Object.freeze([
  freezeArchetype({
...
export const BUILD_ARCHETYPE_BY_ID: Readonly<Record<string, BuildArchetypeProfile>> = Object.freeze(
  Object.fromEntries(SEMESTER_BUILD_ARCHETYPES.map((profile) => [profile.id, profile])) as Record<
    string,
    BuildArchetypeProfile
  >,
);

export function getBuildArchetype(id: string | null): BuildArchetypeProfile | null {
  if (id === null) {
    return null;
  }
...
export function getBuildArchetypesForPath(path: CultivationPath | null): BuildArchetypeProfile[] {
  if (path === null) {
    return [];
  }
