...
function randomFromList<T>(random: RewardRandomSource, list: T[]): T | null {
  if (!Array.isArray(list) || list.length === 0) return null;
  const index = Math.floor(random.next() * list.length);
  return list[index] ?? null;
}

function pickWeighted<T extends { weight: number }>(random: RewardRandomSource, pool: T[]): T | null {
...
function mergeCurrencies(target: RewardBundle['currencies'], source: RewardBundle['currencies']): RewardBundle['currencies'] {
  if (!target && !source) return undefined;
  const next = { ...(target ?? {}) };
  Object.entries(source ?? {}).forEach(([key, value]) => {
    const current = Number(next[key as keyof typeof next] ?? '0');
    const addition = Number(value ?? '0');
    if (Number.isFinite(addition) && addition > 0) {
      next[key as keyof typeof next] = (current + addition).toString() as never;
    }
  });
  return next;
}

export function mergeRewardBundles(...bundles: Array<RewardBundle | undefined>): RewardBundle {
  ...
  bundles.forEach((bundle) => {
    ...
    if (bundle.items?.length) items.push(...bundle.items.map((entry) => ({ ...entry })));
  ...
  if (items.length > 0) merged.items = collapseItems(items);
  return merged;
}
...
export function getOutskirtsDropsConfig(economy: EconomyConfig | null | undefined): OutskirtsDropsConfig | undefined {
  return economy?.drops?.outskirts ?? economy?.manualSystem?.drops?.outskirts;
}

export function getRuinsDropsConfig(economy: EconomyConfig | null | undefined): RuinsDropsConfig | undefined {
  return economy?.drops?.ruins ?? economy?.manualSystem?.drops?.ruins;
}

export function buildOutskirtsRewardBundle(
...
export function rollRuinDropTable(table: RuinDropTable, label: string, random: RewardRandomSource = mathRandomSource): RewardBundle {
  ...
  if (goldMin > 0 || goldMax > 0) {
    ...
    if (gold > 0) bundle.currencies = { ...bundle.currencies, gold: gold.toString() };
  }
  ...
  (table.guaranteed ?? []).forEach((entry) => {
    if (!entry?.itemId || typeof entry.qty !== 'number' || entry.qty <= 0) return;
    items.push({ itemId: entry.itemId, qty: entry.qty });
  });

  const filtered = items.filter((item) => {
    if (!item.itemId.startsWith('gate_')) return true;
...
export function buildRuinsFinalChestBonusBundle(
  ...
  const runeDustRange = valueByIndex<[number, number] | undefined>(dropsConfig.finalChestRuneDustRangeByCityIndex, idx, undefined);
  if (runeDustRange) {
    const qty = randomIntInRange(random, runeDustRange, runeDustRange);
    if (qty > 0) items.push({ itemId: 'mat_rune_dust', qty });
  }

  const artifactRange = valueByIndex<[number, number] | undefined>(dropsConfig.finalChestArtifactShardsRangeByCityIndex, idx, undefined);
  if (artifactRange) {
