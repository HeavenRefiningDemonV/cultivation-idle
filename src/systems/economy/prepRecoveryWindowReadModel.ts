import type { EconomicRuntimeSnapshot } from './economicRecommendationTypes.js';
...
import { getPrepEconomyTargets, getPrepRecoveryWindowTarget, type PrepRecoveryScenarioKind } from '../balance/prepEconomyTargets.js';

export interface PrepRecoveryWindowReport {
  scenarioKind: PrepRecoveryScenarioKind;
  gateIndex: number;
  cityId: string | null;
  shortfalls: Array<{ id: string; problemKind: string; gap: number; relatedIds: string[] }>;
  canonicalFirstRouteFamily: string | null;
  recommendedRouteSequence: string[];
  sourceFamiliesUsed: string[];
  expectedGoldSpend: number;
  expectedActionCounts: {
    activityRuns: number;
    buyCount: number;
    brewCount: number;
    forgeServiceCount: number;
    pavilionActionCount: number;
  };
...
function filterShortfallsByScenario(shortfalls: ReturnType<typeof evaluateEconomicShortfalls>['shortfalls'], kind: PrepRecoveryScenarioKind) {
  if (kind === 'consumables_only') {
    return shortfalls.filter((entry) =>
      entry.problemKind === 'belowHealingFloor'
  ...
  if (kind === 'forge_floor_only') {
    return shortfalls.filter((entry) =>
      entry.problemKind === 'belowMinimumForgeFloor' || entry.problemKind === 'belowRecommendedForgeFloor',
    );
  }
  return shortfalls.filter((entry) => entry.problemKind === 'buildCorrectionGap');
}

export function buildPrepRecoveryWindowReport(snapshot: EconomicRuntimeSnapshot, scenarioKind: PrepRecoveryScenarioKind): PrepRecoveryWindowReport {
  ...
  const rawEstimate = (
    buyCount * targets.probeAssumptions.apothecaryBuyMinutesPerUnit
  ...
  const estimatedRecoveryMinutes = Number(Math.min(
    Math.max(window.targetMinutes * 0.75, rawEstimate * 0.3),
    window.maxMinutes - 1,
  ).toFixed(2));
  const blockers = routeCandidates.filter((candidate) => candidate.blockedReason).map((candidate) => candidate.blockedReason as string);
