...
import type { EconomicRuntimeSnapshot, EconomicSnapshotExpeditionState, EconomicSnapshotShopLine } from './economicRecommendationTypes.js';
...
function buildOwnedItemCountsById(content: ValidatedContent) {
  const bestSourceIndex = buildBestSourceIndex(content);
  const inventory = useInventoryStore.getState();
  return Object.fromEntries(
    bestSourceIndex.scopeTargetIds
      .filter((targetId) => !['gold', 'merit', 'spiritStones'].includes(targetId))
      .map((targetId) => [targetId, inventory.getQty(targetId)]),
  );
}

function buildShopSnapshot(content: ValidatedContent, currentCityId: string | null): EconomicRuntimeSnapshot['currentCityShop'] {
  const shopStore = useShopStore.getState();
  const shop = getApothecaryShopByCityId(content, currentCityId);
  const lines: EconomicSnapshotShopLine[] = shop
  ...
  return {
    ...
    lines,
    purchasedTodayByStockId: Object.fromEntries(
      lines.map((line) => [line.stockId, line.purchasedToday]),
    ),
  };
}

function buildExpeditionSnapshot(): EconomicSnapshotExpeditionState {
...
export function buildEconomicRuntimeSnapshotFromState(input: {
  ...
  currentTrialProgressById?: ReturnType<typeof useTrialStore.getState>['progressByTrialId'];
  ...
  const currentModuleAvailability = (currentCity?.modules ?? []) as LiveWorldModuleKey[];
  ...
  const spendPolicy = getSpendOrderPolicy({
    ...
    currentCityId: input.currentCityId as never,
    selectedPath: input.selectedPath as never,
    currentGateResolved: phase.currentGateResolved,
  });
  const shop = getApothecaryShopByCityId(input.content, input.currentCityId);
  const shopLines: EconomicSnapshotShopLine[] = shop
    ? shop.stock.map((entry) => {
...
export function buildLiveEconomicRuntimeSnapshot(): EconomicRuntimeSnapshot {
