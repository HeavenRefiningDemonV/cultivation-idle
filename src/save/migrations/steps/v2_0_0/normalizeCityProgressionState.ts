...
const summarizeUnlockedCities = (cityIds: string[]) => (cityIds.length > 0 ? cityIds.join(', ') : 'none');

export const v2_0_0_normalize_city_progression_state: MigrationStep = {
  ...
  run: (save) => {
    ...
    const warnings = didMutate
      ? [
          warning(
            'CITY_PROGRESSION_NORMALIZED',
            'City progression state was backfilled or corrected to match canonical realm-entry unlock truth.',
            '1.5',
            'info',
            'cityState',
          ),
    ...
    if (!existingCityState) {
      warnings.push(
        warning(
          'CITY_STATE_MISSING',
          'Save omitted cityState; migration backfilled canonical unlocked cities, current city, and selected modules.',
          '1.5',
          'warning',
          'cityState',
        ),
    ...
    const touched = [
      ...
      touch(
        ...
        didMutate ? 'set' : 'inspect',
        String(normalizedCityState.currentCityId ?? 'null'),
      ),
      touch(
        'cityState.selectedModuleByCity',
        didMutate ? 'set' : 'inspect',
        `Modules set for ${Object.keys(normalizedCityState.selectedModuleByCity).length} unlocked cities.`,
      ),
    ];
    ...
    return createStepResult(
      ...
      didMutate
        ...
        : 'City progression state already matches canonical packet-1.5 truth.',
      { warnings, touchedFieldPaths: touched, didMutate },
    );
  },
};
