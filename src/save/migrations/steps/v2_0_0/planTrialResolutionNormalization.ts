import type { MigrationStep, PlannedMutation } from '../../migrationTypes.js';
...
export const v2_0_0_plan_trial_resolution_normalization: MigrationStep = {
  ...
  run: (save, ctx) => {
    ...
    const touched = [
      ...
      touch(
        `trialState.progressByTrialId.${FIRST_TRIAL_ID}`,
        'inspect',
        `Resolution=${String(firstTrialResolution ?? (firstTrialCleared ? 'cleared' : 'none'))}.`,
      ),
    ];
    ...
    if (currentRealmIndex >= 1 && !firstTrialResolved && !hasFirstGateProof) {
      progressByTrialId[FIRST_TRIAL_ID] = {
        ...
        eligibleFailures:
          typeof firstTrialProgress?.eligibleFailures === 'number'
        ...
        attemptStartAt: null,
        lastAttemptSummary:
          isRecord(firstTrialProgress?.lastAttemptSummary) ? firstTrialProgress.lastAttemptSummary : null,
      };
      ...
      warnings.push(
        ...
        warning('LEGACY_PROGRESS_WITHOUT_GATE_PROOF', 'Legacy progression was already ahead of first-gate proof state; migration normalized the gate to an honest bypassed resolution.', '1.4', 'info', 'trialState.progressByTrialId'),
        warning('GATE_STATE_NORMALIZED_TO_BYPASS', 'First gate normalized to bypassed so the save stays truthful without fabricating a combat clear.', '1.4', 'info', `trialState.progressByTrialId.${FIRST_TRIAL_ID}`),
      );
      touched.push(
        touch(`trialState.progressByTrialId.${FIRST_TRIAL_ID}.resolution`, 'set', 'Normalized to bypassed for legacy already-advanced progression.'),
        touch(`trialState.progressByTrialId.${FIRST_TRIAL_ID}.bypassedAt`, 'set', `Stamped bypass normalization time ${ctx.nowMs}.`),
      );
    }
    ...
    return createStepResult(
      ...
      warnings.length > 0
        ...
        : 'No trial/gate mismatch detected.',
      { warnings, touchedFieldPaths: touched, plannedMutations, didMutate: warnings.length > 0 },
    );
  },
};
