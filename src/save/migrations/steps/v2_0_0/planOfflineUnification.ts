...
export const v2_0_0_plan_offline_unification: MigrationStep = {
  ...
  run: (save) => {
    ...
    ].filter((entry): entry is ReturnType<typeof touch> => entry !== null);
    const warnings = [];
    ...
    if (result.hasSplit && result.normalizedTimestamp !== null) {
      warnings.push(
        ...
        warning(
          'OFFLINE_NORMALIZED_TO_CANONICAL_TIMESTAMP',
          `Packet 1.8 will normalize offline metadata to ${String(result.normalizedTimestamp)} using the latest valid timestamp.`,
          '1.8',
          'info',
          'meta.lastActiveAtMs',
        ),
      ...
      plannedMutations.push(
        plan('meta.lastActiveAtMs', '1.8', 'Choose the latest valid offline timestamp as canonical current-save truth.'),
        plan('gameState.lastActiveTime', '1.8', 'Align runtime-facing offline aliases to the canonical timestamp.'),
        plan('gameState.lastTickTime', '1.8', 'Align runtime-facing offline aliases to the canonical timestamp.'),
      );
      touched.push(
        touch('meta.lastActiveAtMs', 'set', `Canonicalized to ${String(result.normalizedTimestamp)}.`),
        touch('gameState.lastActiveTime', 'set', `Aligned to ${String(result.normalizedTimestamp)}.`),
        touch('gameState.lastTickTime', 'set', `Aligned to ${String(result.normalizedTimestamp)}.`),
      );
    }
    ...
    return createStepResult(
      ...
      result.save,
      result.hasSplit && result.normalizedTimestamp !== null
        ? `Offline split metadata normalized to canonical timestamp ${String(result.normalizedTimestamp)}.`
        : 'Offline metadata already coherent.',
      { warnings, touchedFieldPaths: touched, plannedMutations, didMutate: result.mutated },
    );
  },
};
