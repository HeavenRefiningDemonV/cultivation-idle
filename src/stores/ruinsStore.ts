...
import { buildRuinsFinalChestBonusBundle, mergeRewardBundles, rollRuinDropTable } from '../systems/economy/activityRewardRuntime.js';
  ...
  immer((set, get) => {
    ...
    return {
      ...
      handleRoomVictory: ({ runId, ruinId, cityId, roomIndex }) => {
        ...
        const perRoomRewards = applyLootBonuses(rollRuinDropTable(ruinDef.dropsPerRoom), 'ruins');
        ...
        if (isFinalRoom) {
          const chestRewards = applyLootBonuses(mergeRewardBundles(
            rollRuinDropTable(ruinDef.finalChestDrops),
            buildRuinsFinalChestBonusBundle({ economy: content.economy, cityIndex: ruinDef.cityIndex ?? 0 }),
          ), 'ruins');
          RewardService.grantRewards(
            chestRewards,
