import {
  validateProgressionFixtureCatalog,
  type FixtureValidationStatus,
} from '../tests/fixtures/progression/index.js';

const args = process.argv.slice(2);
const jsonMode = args.includes('--json');
const matrixMode = args.includes('--matrix');

const priority: Record<FixtureValidationStatus, number> = {
  clean: 0,
  warning: 1,
};
...
const main = async () => {
  ...
  const failures = results.filter((result) => {
    if (result.status !== result.fixture.metadata.expectedValidationStatus) return true;
