...
const snapshotRecipeMastery = () => ({ ...useRecipeMasteryStore.getState().alchemy });
const applyMasteryRetention = (carryOver, snapshots) => {
    if (carryOver <= 0)
        return;
    const retainedTechniqueXp = Object.fromEntries(Object.entries(snapshots.techniqueXp)
    ...
    if (Object.keys(retainedTechniqueXp).length > 0) {
        useTechCollectionStore.setState((state) => ({
            ...state,
            unlockedTechs: Object.fromEntries(Object.entries(retainedTechniqueXp).map(([techId, masteryXp]) => [
                techId,
                {
                    unlocked: false,
                    masteryXp,
    ...
    const retainedRecipeMastery = Object.fromEntries(Object.entries(snapshots.recipeMastery)
...
export function performPrestigeReset({ resetGameRun }) {
