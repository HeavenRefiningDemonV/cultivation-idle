...
import { BALANCE_TELEMETRY_SCHEMA_VERSION, } from './balanceTelemetrySchema.js';
...
let nextEventId = 1;
function classifySourceKind(reason) {
    const r = reason.toLowerCase();
    if (r.startsWith('outskirts victory'))
        return 'outskirts';
    if (r.includes('ruins room'))
        return 'ruins_room';
    if (r.includes('ruins chest'))
        return 'ruins_chest';
    if (r.includes('ruins rare'))
        return 'ruins_rare';
    if (r.startsWith('bounty:'))
        return 'bounty';
    if (r.startsWith('expedition_claim:'))
        return 'expedition';
    if (r === 'gate trial clear')
        return 'trial_clear';
    if (r.startsWith('gate trial eligible defeat:'))
        return 'eligible_defeat_merit';
    if (r.startsWith('manual pavilion'))
        return 'manual_pavilion';
    if (r.startsWith('alchemy:') || r.startsWith('talisman:') || r.startsWith('forge session:') || r.startsWith('alchemy session:'))
        return 'craft_completion';
    if (r.startsWith('debug:'))
        return 'debug';
    return 'other';
}
function classifySinkKind(reason) {
    const r = (reason ?? '').toLowerCase();
    if (r.startsWith('shop:'))
        return 'apothecary_shop';
    if (r.startsWith('apothecary_bundle:'))
        return 'apothecary_bundle';
    if (r.startsWith('alchemy_queue:'))
        return 'alchemy_queue';
    if (r.startsWith('talisman_queue:'))
        return 'talisman_queue';
    if (r.startsWith('forge:'))
        return 'forge_service';
    if (r.startsWith('craftsession:'))
        return 'craft_session';
    if (r.startsWith('gate_fail_safe:'))
        return 'gate_fail_safe_purchase';
    if (r.startsWith('prestige_purchase:'))
        return 'prestige_purchase';
    if (r.startsWith('heartlaw:'))
        return 'heart_law_change';
    if (r.startsWith('breakthrough_gate_item:'))
        return 'breakthrough_gate_item';
    return 'other';
}
function deriveContext() {
    ...
    const sessionKind = prestige.prestigeCount > 0 ? 'reclaim' : 'first_life';
    return {
        emittedAt,
...
function emitBalanceEvent(event, contextOverrides) {
    ...
    if (normalized.sessionKind !== 'reclaim')
        return;
...
function emitFromRuntimeEvent(event) {
    switch (event.type) {
...
export function getTrialGateIndex(trialId) {
...
export function initializeBalanceTelemetry() {
    if (initialized)
        return;
    initialized = true;
    GameEvents.onAny(emitFromRuntimeEvent);
}
export function buildTrialAttemptId(trialId, startedAt) {
    return `${trialId}:${startedAt}`;
}
export function getEligibleFailCount(trialId) {
