 ...
 */
export function getAvailablePerks(path, realmIndex) {
    if (!path)
        return [];
    return PATH_PERKS.filter((perk) => perk.path === path && perk.requiredRealm === realmIndex);
}
/**
 * Get a perk by its ID
 */
export function getPerkById(perkId) {
