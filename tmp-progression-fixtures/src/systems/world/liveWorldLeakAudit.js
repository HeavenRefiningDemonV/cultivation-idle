import { DEFERRED_WORLD_MODULES, isDeferredWorldModule, isLiveWorldModule, } from './liveWorldSchema.js';
export const LEGACY_SEMESTER_CITY_NAME_REPLACEMENTS = {
    Embermist: 'Spirit Cavern',
    Silverkeep: 'Lotusford',
    Starsea: 'Ironpeak',
};
const LEGACY_CITY_NAMES = Object.keys(LEGACY_SEMESTER_CITY_NAME_REPLACEMENTS);
const escapeRegExp = (value) => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
export function findLegacySemesterCityNames(text) {
    if (typeof text !== 'string' || text.trim().length === 0) {
        return [];
    }
    return LEGACY_CITY_NAMES.filter((name) => new RegExp(`\\b${escapeRegExp(name)}\\b`, 'i').test(text));
}
export function inspectWorldFacingText(text) {
...
export function isAllowedLiveWorldSurfaceModule(moduleKey) {
    if (typeof moduleKey !== 'string' || moduleKey.length === 0)
        return false;
    return isLiveWorldModule(moduleKey);
}
export function inspectWorldFacingModuleTarget(moduleKey) {
...
export function inspectHiddenWorldModuleSet(hiddenModules) {
    ...
    const canonical = new Set(DEFERRED_WORLD_MODULES);
...
export function inspectWorldFacingModuleList(moduleKeys) {
    const deferred = [];
    const unknown = [];
    for (const moduleKey of moduleKeys ?? []) {
        if (typeof moduleKey !== 'string' || moduleKey.length === 0)
            continue;
        if (isDeferredWorldModule(moduleKey)) {
            if (!deferred.includes(moduleKey))
                deferred.push(moduleKey);
            continue;
        }
