...
import { buildStatusTroubleshootingSurface } from '../ui/status/statusTroubleshootingSurface.js';
import { getTrialGateItemId } from '../progression/runtime/gateResolver.js';
import { buildPostFailureDiagnosisSurface } from '../ui/postFailure/index.js';
function asLiveTrialId(trialId) {
    return SEMESTER_SLICE_CONTRACT.liveTrialIds.includes(trialId) ? trialId : null;
}
function buildPatchedProgressByTrialId(trialId) {
    ...
    if (targetIndex < 0)
        return null;
    return Object.fromEntries(SEMESTER_SLICE_CONTRACT.liveTrialIds.map((currentTrialId, index) => {
...
export function getSection5ReadinessDisplay(band) {
    if (!band)
        return null;
    return { band, label: getReadinessBandLabel(band) };
}
export function getSection5DiagnosisDisplay(code) {
    if (!code)
        return null;
    return { code, label: getDiagnosisLabel(code) };
}
export function buildTrialGateReadinessResult(trialId, snapshot) {
    try {
        const resolvedTrialId = asLiveTrialId(trialId);
        if (resolvedTrialId == null)
            return null;
        ...
        if (patchedProgressByTrialId == null)
            return null;
...
export function buildSection5ReadinessSurface(trialId, snapshot) {
    try {
        const resolvedTrialId = asLiveTrialId(trialId);
        if (resolvedTrialId == null)
            return null;
...
export function buildSection5StatusSurface(snapshot) {
    try {
        const resolvedSnapshot = snapshot ?? buildDoctrineSnapshot();
        const build = analyzeSelectedBuild(resolvedSnapshot);
...
export function buildSection5StatusTroubleshootingSurface() {
    return buildStatusTroubleshootingSurface();
}
export function buildSection5PostFailureSurface(trialId) {
...
function summarizeBuildReadiness(readiness, target) {
    if (!readiness) {
...
function summarizeForgeReadiness(readiness, target) {
    if (!readiness) {
...
function summarizePreparationReadiness(readiness, target) {
    if (!readiness) {
...
function summarizePostureReadiness(readiness, target) {
    if (!readiness) {
...
export function buildGateTrialReadinessSurface(trialId) {
    const readinessSurface = buildSection5ReadinessSurface(trialId);
    const content = useContentStore.getState().raw;
    const maps = useContentStore.getState().maps;
    const resolvedTrialId = asLiveTrialId(trialId);
    if (!readinessSurface || !content || !resolvedTrialId)
        return null;
    const game = useGameStore.getState();
...
export function buildGateTrialAttemptPresentation(surface) {
