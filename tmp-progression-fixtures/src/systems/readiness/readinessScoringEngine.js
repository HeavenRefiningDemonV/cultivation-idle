import { READINESS_SHORTFALL_ORDER, } from './readinessScoringTypes.js';
function countEquippedTechniquesBySlotType(equippedTechniques, slotType) {
    return equippedTechniques.filter((tech) => tech.slotType === slotType).length;
}
function countEquippedTechniquesAtMasteryLevel(equippedTechniques, masteryLevel) {
    return equippedTechniques.filter((tech) => tech.masteryLevel >= masteryLevel).length;
}
function sumRankUpTotal(equippedTechniques) {
    return equippedTechniques.reduce((total, tech) => total + Math.max(0, tech.rank - 1), 0);
}
function sumAppliedRunes(equippedTechniques) {
    return equippedTechniques.reduce((total, tech) => total + tech.appliedRuneCount, 0);
}
function meetsMasteryTargets(equippedTechniques, masteryTargets) {
...
function createShortfall(input) {
    return { ...input };
}
function sortReadinessShortfalls(shortfalls) {
    return [...shortfalls].sort((left, right) => READINESS_SHORTFALL_ORDER.indexOf(left.code) - READINESS_SHORTFALL_ORDER.indexOf(right.code));
}
export function evaluateBuildReadiness(input) {
    ...
    const minimumMet = currentActiveFilled >= gateBuildFloor.minimum.activeSlotsFilled &&
    ...
    const recommendedMet = currentActiveFilled >= gateBuildFloor.recommended.activeSlotsFilled &&
    ...
    const minimumFilledTarget = gateBuildFloor.minimum.activeSlotsFilled +
        gateBuildFloor.minimum.passiveSlotsFilled +
        (gateBuildFloor.minimum.ultimateRequired ? 1 : 0);
    const recommendedFilledTarget = gateBuildFloor.recommended.activeSlotsFilled +
        gateBuildFloor.recommended.passiveSlotsFilled +
        (gateBuildFloor.recommended.ultimateRequired ? 1 : 0);
    if (currentActiveFilled < gateBuildFloor.minimum.activeSlotsFilled ||
        currentPassiveFilled < gateBuildFloor.minimum.passiveSlotsFilled ||
        (gateBuildFloor.minimum.ultimateRequired && currentUltimateFilled < 1)) {
...
export function evaluateForgeReadiness(input) {
    ...
    const minimumMet = currentWeaponRefine >= forgeTargets.minimum.weaponRefine &&
    ...
    const recommendedMet = currentWeaponRefine >= forgeTargets.recommended.weaponRefine &&
    ...
    const minimumTarget = forgeTargets.minimum.weaponRefine +
        forgeTargets.minimum.accessoryRefine +
        forgeTargets.minimum.temperSuccesses +
        forgeTargets.minimum.runeCount;
    const recommendedTarget = forgeTargets.recommended.weaponRefine +
...
export function postureRatingToReadinessBand(rating) {
    if (rating === 'good')
        return 'recommended_met';
    if (rating === 'risky')
        return 'minimum_met_below_recommended';
    return 'below_minimum';
}
export function evaluatePostureReadiness(postureFit) {
...
export function evaluateOverallReadinessBand(input) {
    ...
    if (componentBands.includes('below_minimum'))
        return 'below_minimum';
    if (componentBands.every((band) => band === 'recommended_met'))
        return 'recommended_met';
    return 'minimum_met_below_recommended';
}
export function scoreGateReadiness(input) {
    ...
    const postureOnlyBelowMinimum = strictOverallBand === 'below_minimum' &&
        posture.band === 'below_minimum' &&
        build.band !== 'below_minimum' &&
        forge.band !== 'below_minimum' &&
        economic.band !== 'below_minimum';
    const overallBand = postureOnlyBelowMinimum
        ? 'minimum_met_below_recommended'
        : strictOverallBand;
