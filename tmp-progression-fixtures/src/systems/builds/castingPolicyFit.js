...
const BASELINE_POLICY_RATINGS = Object.freeze({
...
export function getDefaultCastingPolicyForAiProfile(profile) {
    switch (profile) {
        case 'survivor':
            return 'defensive';
        case 'burst':
...
export function evaluateCastingPolicyFit(input) {
    ...
    if (input.encounterType === 'trial' &&
        ...
        rating = 'bad';
        pushWarning(warnings, 'Aggressive casting is a bad fit for a gate trial without both survival and boss-pressure tools.');
    }
    if (input.encounterType === 'ruins' &&
        input.castingPolicy === 'aggressive' &&
        !input.loadoutSignals.hasSurvivalTool) {
        rating = 'bad';
        pushWarning(warnings, 'Aggressive casting is risky in ruins when the loadout lacks survival support.');
    }
    if (SERIOUS_COMBAT_ENCOUNTERS.includes(input.encounterType) &&
        input.castingPolicy === 'defensive' &&
        !input.loadoutSignals.hasSurvivalTool) {
        rating = downgradeRating(rating);
        pushWarning(warnings, 'Defensive casting is under-supported: the loadout lacks real survival tools.');
    }
    if (input.encounterType === 'outskirts' && input.castingPolicy === 'defensive') {
        if (rating === 'good') {
            rating = 'risky';
        }
        pushWarning(warnings, 'Defensive casting slows open-world clears when the current loadout is not under real pressure.');
    }
    if ((input.aiProfile === 'survivor' && input.castingPolicy === 'aggressive') ||
        (input.aiProfile === 'farmer' && input.castingPolicy === 'defensive')) {
        rating = downgradeRating(rating);
        pushWarning(warnings, "Casting policy diverges from the selected AI profile's natural posture.");
    }
    return {
        rating,
        warnings: [...warnings],
