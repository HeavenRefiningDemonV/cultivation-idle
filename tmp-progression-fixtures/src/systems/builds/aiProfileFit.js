...
const FALLBACK_AI_RECOMMENDATIONS = Object.freeze({
    outskirts: Object.freeze(['balanced', 'farmer']),
    ruins: Object.freeze(['balanced', 'survivor']),
    trial: Object.freeze(['survivor', 'balanced']),
});
...
function getRecommendedPair(path, encounterType) {
...
export function evaluateAiProfileFit(input) {
    ...
    let rating = input.aiProfile === primaryRecommendation || input.aiProfile === secondaryRecommendation
        ? 'good'
        : 'risky';
    ...
    if (input.encounterType === 'outskirts' &&
        ...
        pushWarning(warnings, 'Farmer AI is under-supported: the current loadout lacks clear-speed tools.');
    }
    if (input.encounterType === 'trial' &&
        input.aiProfile === 'burst' &&
        !input.loadoutSignals.hasBossTool) {
        rating = downgradeRating(rating);
        pushWarning(warnings, 'Burst AI is under-supported: no boss-pressure tool is currently equipped.');
    }
    if (SERIOUS_COMBAT_ENCOUNTERS.includes(input.encounterType) &&
        input.aiProfile === 'survivor' &&
        !input.loadoutSignals.hasSurvivalTool) {
        rating = downgradeRating(rating);
        pushWarning(warnings, 'Survivor AI is under-supported: no survival tool is currently equipped.');
    }
    if (input.encounterType === 'trial' &&
        ...
        !input.loadoutSignals.hasBossTool) {
        rating = downgradeRating(rating);
        pushWarning(warnings, 'Balanced AI is under-supported: the current loadout lacks both survival and boss-pressure tools.');
    }
    return {
        rating,
        warnings: [...warnings],
