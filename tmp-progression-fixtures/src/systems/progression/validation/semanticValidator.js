import { adaptProgressionAuthoredContent, buildProgressionContract, getContentCapRealm, isRefundableHiddenPrestigeNode, normalizeGateItemAlias, } from '../contract/index.js';
import { collectProgressionDiagnostics } from '../diagnostics/index.js';
import { inspectLiveCitySchema, LIVE_CITY_MODULE_ORDER } from '../../world/liveWorldSchema.js';
import { formatCityPackageCoverageReport, inspectSemesterCityPackageCoverage, } from '../../world/cityPackageRegistry.js';
...
const readTrialFromMajorRealm = (eligibilityRule) => {
    if (!eligibilityRule)
        return undefined;
    if (typeof eligibilityRule === 'object') {
        const record = eligibilityRule;
...
const buildAuthoredDiagnosticsInput = (content) => ({
...
const readRawCities = (rawContent) => Array.isArray(rawContent.cities) ? rawContent.cities : rawContent.cities.cities;
const readRawOutskirts = (rawContent) => Array.isArray(rawContent.outskirts) ? rawContent.outskirts : rawContent.outskirts?.outskirts ?? [];
const readRawRuins = (rawContent) => Array.isArray(rawContent.ruins) ? rawContent.ruins : rawContent.ruins?.ruins ?? [];
const readRawPavilions = (rawContent) => Array.isArray(rawContent.pavilions) ? rawContent.pavilions : rawContent.pavilions?.pavilions ?? [];
const readRawApothecaryShops = (rawContent) => Array.isArray(rawContent.apothecary_shops) ? rawContent.apothecary_shops : rawContent.apothecary_shops?.shops ?? [];
const validateRawRewardParity = (rawContent) => {
    const issues = [];
...
const validateRawWorldCitySchema = (rawContent) => {
    const issues = [];
...
const validateRawWorldCityPackageCoverage = (rawContent) => {
    const cities = readRawCities(rawContent).map((city) => ({
        id: city.id,
        index: city.index ?? 0,
...
const legacyGateItemEntries = (items) => Object.keys(items).filter((itemId) => !itemId.startsWith('gate_') && normalizeGateItemAlias(itemId) !== null);
const validateScenarioSemantics = (contract, scenarios) => {
    const issues = [];
    scenarios.forEach((scenario) => {
        ...
        if (!contract.majorRealms[scenario.realmState.currentRealm]) {
            pushIssue(issues, {
...
const readMigrationFixtureIssues = (contract, migrationFixtures) => {
    const issues = [];
    ...
    migrationFixtures.forEach(({ name, data }) => {
        ...
        const itemIds = Object.keys(asRecord(inventoryState.items));
        const prestigePurchases = asRecord(prestigeState.purchasesById);
        const offlineTimes = [metaState.lastActiveAtMs, gameState.lastActiveTime, gameState.lastTickTime].filter((value) => typeof value === 'number');
        const unlockedCityIds = Array.isArray(cityState.unlockedCityIds)
            ? cityState.unlockedCityIds.filter((cityId) => typeof cityId === 'string')
            : [];
        const currentCityId = typeof cityState.currentCityId === 'string' ? cityState.currentCityId : null;
        if (typeof gameState.lifePath === 'string' &&
        ...
        const cityProgressionMismatch = canonicalUnlockedCities.length > 0 &&
            (!record.cityState ||
        ...
        const hasPartialResetResidue = realmIndex === 0 &&
            (Array.isArray(cityState.unlockedCityIds) && cityState.unlockedCityIds.length > 1 ||
                ...
                Object.values(asRecord(equipmentState.refineLevelBySlot)).some((value) => Number(value) > 0));
...
export const validateProgressionSemantics = (options) => {
