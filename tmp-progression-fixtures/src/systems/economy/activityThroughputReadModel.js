import { ACTIVITY_THROUGHPUT_VALIDATION_THRESHOLDS, getActivityProbeDurationSeconds, } from '../balance/activityThroughputTargets.js';
import { classifyActivityRewardItem, getCityRewardRoleProfile } from './activityRewardRoles.js';
import { buildOutskirtsRewardBundle, buildRuinsFinalChestBonusBundle, getOutskirtsDropsConfig, getRuinsDropsConfig, mergeRewardBundles, rollRuinDropTable, } from './activityRewardRuntime.js';
class SeededRandom {
    state;
    constructor(state) {
        this.state = state;
...
const createBucketSummary = () => ({ gold: 0, common: 0, targeted: 0, anchor: 0, support: 0, other: 0 });
const summarizeBundle = (cityId, bundle) => {
    ...
    bundle.items?.forEach((item) => {
        ...
        if (bucket === 'common_field')
            summary.common += item.qty;
        else if (bucket === 'targeted_local')
            summary.targeted += item.qty;
        else if (bucket === 'anchor')
            summary.anchor += item.qty;
        else if (bucket === 'support')
            summary.support += item.qty;
        else
            summary.other += item.qty;
    });
    return summary;
};
...
const divideSummary = (summary, denominator) => {
...
function valueByIndex(source, index, fallback) {
    ...
    if (source && typeof source === 'object')
        return source[index] ?? Object.values(source).at(-1) ?? fallback;
    return fallback;
}
...
export function buildOutskirtsThroughputSnapshot(content, cityId) {
    const outskirts = content.outskirts.find((entry) => entry.cityId === cityId);
    if (!outskirts)
        throw new Error(`[ActivityThroughputReadModel] Missing outskirts for ${cityId}`);
    ...
    const expectedBossSpiritStoneContribution = valueByIndex(drops?.bossSpiritStoneChanceByCityIndex, cityIndex, 0) *
...
export function buildRuinsThroughputSnapshot(content, cityId) {
    const ruin = content.ruins.find((entry) => entry.cityId === cityId);
    if (!ruin)
        throw new Error(`[ActivityThroughputReadModel] Missing ruins for ${cityId}`);
    ...
    return {
        ...
        roleShare: {
            targetedAndAnchorShare: (runTotals.targeted + runTotals.anchor) /
...
export function buildActivityThroughputCitySnapshot(content, cityId) {
    ...
    return {
        ...
        ruins,
        roleChecks: {
            outskirtsGoldDominance: outskirts.goldPerMinute >= ruins.goldPerMinute * ACTIVITY_THROUGHPUT_VALIDATION_THRESHOLDS.outskirtsGoldPerMinuteDominanceMinRatio,
            outskirtsCommonDominance: outskirts.commonMaterialUnitsPerMinute >=
                ruins.commonMaterialUnitsPerMinute * ACTIVITY_THROUGHPUT_VALIDATION_THRESHOLDS.outskirtsCommonPerMinuteDominanceMinRatio,
            ruinsTargetedDominance: ruins.targetedMaterialUnitsPerMinute >=
            ...
            outskirtsSupportSecondary: outskirts.bossSpiritStoneSupportPerHour <= outskirts.goldPerMinute * ACTIVITY_THROUGHPUT_VALIDATION_THRESHOLDS.outskirtsSupportVsPrimaryGoldMaxShare,
        },
    };
}
export function buildAllActivityThroughputSnapshots(content) {
