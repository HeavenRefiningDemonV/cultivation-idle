...
const getWeight = (upgradeId) => PRESTIGE_TARGETS.starterSpendCadencePolicy.reclaimScoreWeightsByUpgradeId[upgradeId] ?? DEFAULT_WEIGHT;
const normalizeCost = (upgrade, nextLevel) => {
    if (nextLevel < 1 || nextLevel > upgrade.maxLevel)
        return null;
    const cost = upgrade.costs?.[nextLevel - 1];
    return typeof cost === 'number' && Number.isFinite(cost) && cost >= 0 ? cost : null;
};
const prereqsMet = (upgrade, purchasedLevels) => (upgrade.prereq ?? []).every((prereq) => (purchasedLevels[prereq.upgradeId] ?? 0) >= prereq.minLevel);
    ...
    while (remainingAp > 0) {
        const candidates = args.visibleUpgrades
            ...
            if (currentLevel >= upgrade.maxLevel)
                return null;
            if (!prereqsMet(upgrade, planningLevels))
                return null;
            const nextCost = normalizeCost(upgrade, currentLevel + 1);
            if (nextCost === null || nextCost > remainingAp)
                return null;
