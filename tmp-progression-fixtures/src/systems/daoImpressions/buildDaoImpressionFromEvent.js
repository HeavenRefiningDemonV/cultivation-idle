...
function normalizedPct(value) {
    if (value == null || !Number.isFinite(value))
        return null;
    return value > 1 ? Math.max(0, Math.min(1, value / 100)) : Math.max(0, Math.min(1, value));
}
export function buildDaoImpressionCandidateFromEvent(event) {
    if (event.type === 'combat/resolved') {
        const payload = event.payload;
        if (!isRecord(payload) || payload.outcome !== 'victory')
            return null;
    ...
    if (event.type === 'trials/attempt_resolved') {
        ...
        if (!trialId || gateIndex === null || !attemptId || timestamp === null)
            return null;
