...
import { CULTIVATION_CONSUMABLE_FAMILY_REGISTRY, DEFAULT_CULTIVATION_CONSUMABLE_MODIFIERS, } from './cultivationConsumableTypes.js';
export function mergeCultivationConsumableModifiers(consumables, now) {
    return consumables.reduce((acc, entry) => {
...
export function buildCultivationConsumableReadModel(consumables, now) {
    ...
    const activeByFamily = active.reduce((acc, entry) => {
...
export function buildCultivationConsumableCarryoverWindows(consumables, startAt, endAt) {
    if (endAt <= startAt)
        return [];
    const relevant = consumables
        .filter((entry) => entry.expiresAt > startAt && entry.activatedAt < endAt)
        .map((entry) => ({ ...entry }));
    const checkpoints = new Set([startAt, endAt]);
    relevant.forEach((entry) => {
        if (entry.activatedAt > startAt && entry.activatedAt < endAt)
            checkpoints.add(entry.activatedAt);
        if (entry.expiresAt > startAt && entry.expiresAt < endAt)
            checkpoints.add(entry.expiresAt);
    });
