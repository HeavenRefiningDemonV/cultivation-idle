...
function patternKindForDiagnosis(code) {
...
function routeTargetFromDestination(destination) {
...
function defaultRouteReason(code, target) {
    if (code === 'close')
        return 'Your pressure is near enough; one cleaner exchange should decide the next attempt.';
    if (target === 'forge')
        return 'Raise the weapon or armor floor before feeding this pattern another attempt.';
    if (target === 'apothecary')
        return 'Stock the medicine posture before returning to the gate.';
    if (target === 'techniques')
        return 'Tune the active build so the gate is testing doctrine, not missing slots.';
    if (target === 'cultivation')
        return 'Settle breath and Heart Law progress before forcing the same threshold.';
    return 'Correct the named pattern before returning to the gate.';
}
function reflectionIdFor(trialId, gateIndex, diagnosisCode) {
    return `inner_demon:${trialId}:${gateIndex}:${normalizeDiagnosisCode(diagnosisCode)}`;
}
function buildReflectionRecord(input, repeatedCount) {
...
export const useFailureReflectionStore = create()((set, get) => ({
    reflections: [],
    recentAttempts: [],
    ...
    recordAttemptTrace: (attempt) => {
        ...
        return get().recentAttempts.filter((trace) => (trace.trialId === normalized.trialId
            && trace.gateIndex === normalized.gateIndex
            && trace.diagnosisCode === normalized.diagnosisCode)).length;
    },
    ...
    resolveForTrial: (trialId, gateIndex, resolvedBy, resolvedAt) => {
        ...
        set((state) => ({
            reflections: state.reflections.map((entry) => {
                if (entry.trialId !== trialId || entry.gateIndex !== gateIndex || entry.resolved)
                    return entry;
                resolved = {
                    ...entry,
                    resolved: true,
    ...
    getActiveReflectionForTrial: (trialId, gateIndex) => (get().reflections.find((entry) => (entry.trialId === trialId
        && !entry.resolved
        && (typeof gateIndex !== 'number' || entry.gateIndex === gateIndex))) ?? null),
...
export function recordFailureReflectionAttempt(input) {
    ...
    if (repeatedCount < DEFAULT_REPEAT_THRESHOLD)
        return null;
...
export function resolveFailureReflectionOnDiagnosisChange(input) {
    ...
    if (!active)
        return null;
    if (active.diagnosisCode === normalizeDiagnosisCode(input.diagnosisCode))
        return null;
...
export function resolveFailureReflectionForTrial(trialId, gateIndex, resolvedBy, resolvedAt = Date.now()) {
    return useFailureReflectionStore.getState().resolveForTrial(trialId, gateIndex, resolvedBy, resolvedAt);
}
export function clearFailureReflections() {
    useFailureReflectionStore.getState().clearCurrentLife();
}
export function hardResetFailureReflections() {
    useFailureReflectionStore.getState().hardResetFailureReflections();
}
export function captureResolvedFailureReflections(limit = 3) {
