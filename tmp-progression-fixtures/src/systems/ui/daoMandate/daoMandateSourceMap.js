const MODULE_DESTINATION_LABELS = {
    outskirts: 'Outskirts',
    ruins: 'Ruins',
...
const SEALED_CRITICAL_KINDS = new Set([
...
function currentCityId(runCompass, route) {
    if (runCompass.currentCity?.cityId)
        return runCompass.currentCity.cityId;
    if (route.target?.kind === 'world_module')
        return route.target.cityId;
    return null;
}
function currentVisibleModules(runCompass) {
    return runCompass.currentCity?.visibleModuleKeys ?? [];
}
function isWorldModuleKey(moduleKey) {
    return moduleKey !== 'techniques' && moduleKey !== 'inventory' && moduleKey !== 'records';
}
function moduleTargetFor(moduleKey, cityId) {
    if (moduleKey === 'techniques' || moduleKey === 'inventory' || moduleKey === 'records') {
        return { kind: 'tab', tab: moduleKey };
    }
    if (!cityId)
        return null;
    return { kind: 'world_module', cityId, moduleKey };
}
function routeTargetsModule(route, moduleKey) {
    if (!route?.target)
        return false;
    if (route.target.kind === 'world_module')
        return route.target.moduleKey === moduleKey;
    return route.target.tab === moduleKey;
}
function targetModuleKey(route) {
    if (!route?.target)
        return null;
    if (route.target.kind === 'world_module')
...
function activityModeForRoute(route) {
    if (route.activityMode === 'background')
        return 'background';
    if (route.activityMode === 'passive')
        return 'passive';
...
function sourceOptionFromRoute(route, label, detail) {
    return {
        id: `source-${route.id}`,
...
function gateSinkLabel(runCompass, fallback) {
    const gateLabel = runCompass.currentGate?.gateLabel;
    if (gateLabel)
        return `${gateLabel} ${fallback}`;
    return `${runCompass.milestone.label} ${fallback}`;
}
function inferNeedDescriptor(runCompass, primaryRoute) {
    const primaryTarget = targetModuleKey(primaryRoute);
    const kind = runCompass.primaryBlocker.kind;
    ...
    if (primaryRoute.source === 'economy' ||
...
function routeDedupKey(route) {
    if (route.target?.kind === 'world_module')
        return `world:${route.target.cityId}:${route.target.moduleKey}`;
    if (route.target?.kind === 'tab')
        return `tab:${route.target.tab}`;
...
function buildFallbackSources(args) {
    ...
    const synthesized = args.descriptor.fallbackModules.map((moduleKey, index) => sourceOptionForModule({
        moduleKey,
        cityId,
        visibleModules,
...
function fallbackDetailFor(kind, moduleKey) {
    if (moduleKey === 'inventory')
        return 'Check owned stock before choosing a new route.';
    if (moduleKey === 'records')
        return 'Use discovered records as evidence for the source pattern.';
    if (moduleKey === 'outskirts')
        return 'Gather broad gold, herbs, or common materials through the field route.';
    if (moduleKey === 'ruins')
        return 'Target scarce materials through the relief route when it is open.';
    if (moduleKey === 'expeditions')
        return 'Use background yield to smooth the shortage while cultivation continues.';
    if (moduleKey === 'bounties')
        return 'Use bounty work when the reward overlaps the current reserve or material route.';
    if (moduleKey === 'manualPavilion')
        return 'Return to doctrine sources if the expression gap is actually missing knowledge.';
    if (moduleKey === 'techniques')
        return 'Refine the known doctrine through loadout, mastery, rank, rune, or posture.';
    if (kind === 'medicine_floor')
        return 'Use this source only if Apothecary stock or brew inputs are thin.';
...
function priorityForEntry(entry) {
    ...
    if (kind === 'required_item_missing')
        return SOURCE_PRIORITY.item;
    if (kind === 'source_route_locked')
        return 15;
    if (kind === 'forge_floor')
        return SOURCE_PRIORITY.forge_floor;
    if (kind === 'medicine_floor')
        return SOURCE_PRIORITY.medicine_floor;
    if (kind === 'manual_knowledge')
        return SOURCE_PRIORITY.manual_knowledge;
    if (kind === 'technique_loadout')
        return SOURCE_PRIORITY.technique_loadout;
    if (kind === 'merit_reserve')
        return SOURCE_PRIORITY.merit_reserve;
    if (kind === 'expedition_yield')
        return SOURCE_PRIORITY.expedition_yield;
    return SOURCE_PRIORITY.unknown;
}
export function buildDaoMandateSourceMap(args) {
...
function isSealedCriticalEntry(entry) {
    if (entry.route?.blocked)
        return true;
    return SEALED_CRITICAL_KINDS.has(entry.problemKind ?? '');
}
export function applyDaoMandateSourceMapProfileVisibility(entries, profile) {
    if (profile === 'sealed') {
        return entries
...
function relationForModule(entry, moduleKey) {
...
function strongestRelation(relations) {
    if (relations.includes('primary-evidence'))
        return 'primary-evidence';
    if (relations.includes('blocked'))
        return 'blocked';
    if (relations.includes('completed'))
        return 'completed';
    if (relations.includes('supporting-source'))
        return 'supporting-source';
    return 'quiet';
}
function moduleHeadline(moduleKey, relation, entry) {
    if (moduleKey === 'inventory')
        return entry ? 'Needed Now' : 'Inventory record';
    if (moduleKey === 'records')
        return entry ? 'Source memory' : 'Source record';
    if (entry)
        return entry.neededThingLabel;
    if (relation === 'blocked')
        return `${MODULE_DESTINATION_LABELS[moduleKey]} source sealed`;
    if (relation === 'completed')
        return `${MODULE_DESTINATION_LABELS[moduleKey]} source settled`;
    return `${MODULE_DESTINATION_LABELS[moduleKey]} support`;
}
function moduleDetail(moduleKey, relation, entry) {
    if (moduleKey === 'records') {
        return entry
    ...
    if (relation === 'blocked')
        return 'The source is known, but the route is blocked from this screen.';
    return 'No visible local source relation.';
}
function confidenceEvidence(entry) {
    if (!entry)
        return [];
    const evidence = new Set([`sourceMap:${entry.id}`]);
    if (entry.problemKind)
        evidence.add(`problemKind:${entry.problemKind}`);
    for (const source of entry.bestSources)
        evidence.add(`bestSource:${source.id}`);
    for (const source of entry.fallbackSources)
        evidence.add(`fallbackSource:${source.id}`);
    return [...evidence];
}
export function buildDaoMandateModuleSourceSinkSurface(args) {
