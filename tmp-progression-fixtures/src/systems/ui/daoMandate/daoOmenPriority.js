export const DAO_OMEN_PRIORITY = {
    life_setup: 10,
    content_cap: 20,
...
export function getDaoOmenPriority(kind) {
    return DAO_OMEN_PRIORITY[kind];
}
export function isDaoOmenOrdinaryPressure(kind) {
    return kind === 'quiet'
...
export function daoOmenRouteTargetsGateTrial(route) {
    if (route.target?.kind === 'world_module')
        return route.target.moduleKey === 'gateTrial';
    if (route.target?.kind === 'tab')
        return false;
    return /gate trial|inspect gate|attempt gate|review gate proof/i.test(routeSearchText(route));
}
export function decideDaoOmenDirectRoute(kind, route, context = {}) {
...
export function normalizeDaoOmenExposedRoute(route, reason, kind) {
    switch (reason) {
        case 'setup':
            return {
                ...route,
...
function daoOmenRouteTargetsBreakthrough(route) {
    if (route.target?.kind !== 'tab')
        return false;
    return route.target.tab === 'cultivation';
}
function daoOmenRouteTargetsPrestigeOrRecords(route) {
    if (route.target?.kind !== 'tab')
        return false;
    return route.target.tab === 'prestige' || route.target.tab === 'records';
}
function setupActionLabel(route) {
    const text = routeSearchText(route);
    if (/heart law/i.test(text))
        return 'Choose Heart Law';
    if (/path/i.test(text))
        return 'Choose Path';
    return 'Review Life Setup';
}
function reflectionRouteLabel(route) {
    const text = routeSearchText(route);
    if (/apothecary|medicine|survival/i.test(text))
        return 'Review survival evidence';
    if (/forge|weapon|gear/i.test(text))
        return 'Review forge pressure';
    if (/technique|manual|doctrine|loadout/i.test(text))
        return 'Review doctrine evidence';
    if (/source|herb|ore/i.test(text))
