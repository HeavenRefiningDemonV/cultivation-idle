...
function sourceMapFromRoute(routeEntry, neededThingLabel) {
    return {
...
function attemptableGateFixture() {
    const primaryRoute = route({
...
function breakthroughPendingFixture() {
    const primaryRoute = route({
...
export function createDaoMandateFixture(state, profile = getDefaultDaoMandateGuidanceProfile()) {
    const raw = (() => {
        switch (state) {
            case 'life_setup':
