...
const WORLD_ROUTE_OPEN_FAILURE_REASON = 'Route could not be opened. The target may be locked, unavailable, or blocked by the current activity.';
export function performDaoMandateRouteAction(route) {
    if (route.blocked) {
    ...
    if (route.target.kind === 'world_module') {
        ...
        const opened = ui.activeTab === 'adventure' &&
    ...
    const _exhaustive = route.target;
