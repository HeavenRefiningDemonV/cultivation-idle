...
export const DAO_OMEN_DEFAULT_COPY = {
    quiet: {
...
const FORBIDDEN_DEFAULT_COPY_REGEX = new RegExp(DAO_OMEN_FORBIDDEN_DEFAULT_COPY_PATTERNS.map(escapeRegExp).join('|'), 'i');
export function getDaoOmenDefaultCopy(kind) {
    return DAO_OMEN_DEFAULT_COPY[kind];
}
export function containsDaoOmenForbiddenDefaultCopy(text) {
    return FORBIDDEN_DEFAULT_COPY_REGEX.test(text);
}
export function assertDaoOmenCopyBudget(entry) {
