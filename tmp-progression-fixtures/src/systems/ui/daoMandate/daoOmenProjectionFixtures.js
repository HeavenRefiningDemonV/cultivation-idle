...
function gateProofMissing(profile) {
...
function medicineShort(profile) {
...
function sourceDrought(profile) {
...
function attemptableGate(profile, risky) {
    const stateId = risky ? 'attemptable_gate_risky' : 'attemptable_gate_viable';
...
function repeatedFailure(profile) {
...
function safetyNetReady(profile) {
...
function breakthroughReady(profile) {
    const primaryRoute = route({
...
function prestigeViable(profile) {
    const primaryRoute = route({
...
function moduleRoute(id, label, destinationLabel, moduleKey, source) {
    return route({
        id,
        label,
        actionLabel: label,
...
function metRow(id, label, proofRoute, priority) {
    return row({
        id,
        bucket: 'hard_gate',
        label,
...
export function createDaoOmenProjectionRawFixture(state, profile = 'jade') {
    switch (state) {
