...
function toneFromRunDelta(tone) {
    if (tone === 'success')
        return 'success';
    if (tone === 'warning')
        return 'warning';
...
function readinessDeltaLabel(delta) {
    if (delta.readinessDelta?.deltaLabel)
        return delta.readinessDelta.deltaLabel;
...
function priorityForRunDelta(delta) {
    const text = `${delta.source} ${delta.label} ${delta.detail} ${delta.memoryLine}`.toLowerCase();
    if (/blocked|safety net|cap recommended|chapter|content cap/.test(text))
        return 0;
    if (/gate.*(clear|opened|proof|bypass)|realm entered|city unlock/.test(text))
        return 5;
    if (/defeat|rejection|inner demon|route changed|mandate shifted/.test(text))
        return 10;
    if (/offline|threshold|source route|resolved/.test(text))
        return 15;
    if (/craft|medicine|forge|manual|technique|floor/.test(text))
        return 20;
    if (/dao impression|impression/.test(text))
        return 30;
    if (/ap|reincarnation|prestige/.test(text))
        return 40;
    return 50;
}
function candidateFromRunDelta(delta) {
    return {
        id: delta.id,
        source: delta.source,
...
function detailForFailure(record) {
...
function candidateFromFailure(record) {
    if (record.resolved || !record.memoryEligible)
        return null;
...
function candidateFromDaoImpression(award) {
    if (!award.memoryEligible)
        return null;
    return {
...
function candidateFromOfflineReturn(args, now) {
    if (args.state === 'unavailable' || args.state === 'unchanged')
        return null;
    return {
...
function dedupeAndSort(candidates, cap) {
    const byTrigger = new Map();
    for (const candidate of candidates) {
        const existing = byTrigger.get(candidate.triggerHash);
        if (!existing || candidate.priority < existing.priority || (candidate.priority === existing.priority && candidate.timestamp >= existing.timestamp)) {
...
export function buildDaoMandateRecentOmens(args) {
    const candidates = [];
    for (const delta of args.runDeltas ?? [])
        candidates.push(candidateFromRunDelta(delta));
