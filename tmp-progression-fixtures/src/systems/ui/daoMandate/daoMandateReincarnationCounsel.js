function stateFromAdvisor(args) {
    if (args.advisorState === 'hidden')
        return 'hidden';
    if (args.route?.blocked && args.advisorState !== 'cap_recommended' && args.advisorState !== 'chapter_exhausted')
        return 'blocked';
    if (args.advisorState === 'chapter_exhausted')
        return 'cap_recommended';
    if (args.advisorState === 'cap_recommended')
        return 'cap_recommended';
    return args.advisorState;
}
function copyForState(state, ap, blockedDetail) {
    if (state === 'hidden') {
...
export function buildDaoReincarnationCounsel(args) {
