...
import { getLiveRealmById, getLiveRealmByIndex, getNextLiveRealm, getTrialGateItemId, isAtSemesterCap, getTrialLifecycleSnapshot, } from '../../progression/runtime/index.js';
...
const PRESTIGE_TARGET = { kind: 'tab', tab: 'prestige' };
function tabRoute(args) {
    const target = { kind: 'tab', tab: args.tab };
    return {
        id: args.id,
        label: args.label,
...
function mapActionKindLabel(actionKind) {
    switch (actionKind) {
        case 'buy':
...
function mapProblemKindToBlockerKind(problemKind) {
    const normalized = problemKind ?? '';
    if (normalized.includes('Forge'))
        return 'forge_floor_shortfall';
    if (normalized.includes('Prep') || normalized.includes('Consumable') || normalized.includes('Medicine'))
        return 'apothecary_prep_shortfall';
    if (normalized.includes('buildCorrection'))
        return 'build_correction_gap';
    if (normalized.includes('Manual'))
        return 'manual_pavilion_gap';
    if (normalized.includes('Merit'))
        return 'bounty_merit_shortfall';
    if (normalized.includes('Expedition'))
        return 'expedition_shortage_smoothing';
    return 'readiness_shortfall';
}
function routeFromCandidate(candidate, index) {
...
function routeForFailureFix(destination, cityId) {
    switch (destination) {
        case 'cultivation':
            return tabRoute({
...
function routeForFailureReflection(target, label, detail, cityId) {
    switch (target) {
        case 'cultivation':
            return tabRoute({
...
function blockerKindForDiagnosis(primary) {
    switch (primary) {
...
function getShortfallLabel(shortfall) {
    if (!shortfall)
        return null;
    return `${shortfall.label} gap ${formatNumber(shortfall.gap)}`;
}
function selectSecondaryRoutes(primary, candidates) {
    ...
    for (const route of candidates.sort((left, right) => left.priority - right.priority)) {
        ...
        if (seen.has(route.id) || seen.has(key))
            continue;
        seen.add(route.id);
        seen.add(key);
        result.push(route);
...
export function buildLiveRunCompassSurfaceV2() {
    ...
    try {
        ...
        const prestigeHint = atCap
            ...
            : canPrestige
                ...
                : null;
        const currentCity = currentCityDef
            ? {
            ...
            : null;
        const currentGate = activeTrial && activeTransition && lifecycle
            ? {
        ...
        const readinessRows = [
            {
                ...
                tone: atRealmEdge ? 'success' : 'default',
            },
            {
                id: 'qi',
                label: 'Qi threshold',
            ...
            },
            {
                ...
                tone: gateReadiness?.readinessLabel === 'Ready' ? 'success' : 'default',
            },
        ];
        const readiness = {
        ...
        if (!game.selectedPath) {
            ...
            primaryRoute = matchingReflection
                ? routeForFailureReflection(matchingReflection.correctiveRoute.target, matchingReflection.correctiveRoute.label, matchingReflection.correctiveRoute.reason, currentCityDef?.id ?? null)
                : topFix
