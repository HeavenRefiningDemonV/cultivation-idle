...
function buildAttemptRecap(summary) {
    if (!summary)
        return null;
    ...
    const biggestHitLine = Number.isFinite(summary.maxHit)
        ...
        : null;
    if (!bossHpRemainingLine && !timeSurvivedLine && !biggestHitLine)
        return null;
    return {
        bossHpRemainingLine,
        timeSurvivedLine,
...
function resolveSecondaryLabel(diagnosis) {
...
function resolveHeadline(primary) {
    switch (primary) {
...
function resolveExplanation(input) {
...
export function buildPostFailureDiagnosisSurface(input) {
