...
import { sanitizeStoredBountyState, useBountyStore } from '../stores/bountyStore.js';
import { useExpeditionStore } from '../stores/expeditionStore.js';
import { getDefaultUnlockedHeartLawIds, useHeartLawStore } from '../stores/heartLawStore.js';
...
let lastLoadedSaveData = null;
let lastLoadMigrationReport = null;
let lastLoadFailure = null;
const recordLoadFailure = (failure) => {
    const enriched = { ...failure, timestamp: Date.now() };
...
const isValidRuinsRunSummary = (value) => {
    ...
    if (typeof record.runId !== 'string')
        return false;
    if (typeof record.ruinId !== 'string')
        return false;
    if (typeof record.startedAt !== 'number' || typeof record.endedAt !== 'number')
        return false;
    if (typeof record.durationSec !== 'number')
        return false;
    if (typeof record.roomsCleared !== 'number' || typeof record.roomCount !== 'number')
        return false;
    if (typeof record.victory !== 'boolean')
        return false;
    if (typeof record.goldGained !== 'number')
        return false;
    if (typeof record.rareDropCount !== 'number')
        return false;
    if (!Array.isArray(record.drops))
        return false;
    return record.drops.every((drop) => {
        ...
        if (typeof d.qty !== 'number')
            return false;
        if ('rarity' in d && d.rarity != null && typeof d.rarity !== 'string')
            return false;
        if ('reason' in d && d.reason != null && typeof d.reason !== 'string')
            return false;
        return true;
    });
};
function cloneManualSatchelState(source) {
    return {
...
function gatherGameState() {
    ...
    const saveData = {
        ...
        shopState: {
            ...
            purchasedToday: Object.fromEntries(Object.entries(shopState.purchasedToday).map(([shopId, entries]) => [
                shopId,
                { ...entries },
            ])),
        },
...
function validateSaveData(data) {
    try {
        if (!data || typeof data !== 'object')
            return false;
        const record = data;
        if (!('version' in record) || !('timestamp' in record))
            return false;
        if (!('gameState' in record) || !('inventoryState' in record) || !('combatSettings' in record))
            return false;
        if ('meta' in record && record.meta !== undefined) {
            const meta = record.meta;
            if (typeof meta.lastActiveAtMs !== 'number')
                return false;
        }
        // Basic structure validation
        const gs = record.gameState;
        if (!('realm' in gs) || typeof gs.qi !== 'string')
            return false;
        if ('spiritRoot' in gs && gs.spiritRoot !== undefined) {
            const sr = gs.spiritRoot;
            if (sr !== null &&
                (!sr || typeof sr.grade !== 'number' || typeof sr.element !== 'string' || typeof sr.purity !== 'number')) {
                return false;
            }
        }
        if ('selectedPath' in gs &&
            gs.selectedPath !== null &&
            gs.selectedPath !== undefined &&
            typeof gs.selectedPath !== 'string') {
            return false;
        }
        if ('lifePath' in gs &&
            gs.lifePath !== null &&
            gs.lifePath !== undefined &&
            typeof gs.lifePath !== 'string') {
            return false;
        }
        if ('lastTickTime' in gs && typeof gs.lastTickTime !== 'number')
            return false;
        if ('lastActiveTime' in gs && typeof gs.lastActiveTime !== 'number')
            return false;
        if ('runStartTime' in gs && typeof gs.runStartTime !== 'number')
            return false;
        if ('currentChapterExhaustedAcknowledgedThisLife' in gs &&
        ...
        if ('prestigeState' in record && record.prestigeState) {
            const ps = record.prestigeState;
            const numbersValid = typeof ps.totalAP === 'number' &&
            ...
            if (!numbersValid)
                return false;
            if (!Array.isArray(ps.prestigeRuns))
                return false;
        ...
        const itemsValue = is.items;
        const itemsValid = (typeof itemsValue === 'object' && itemsValue !== null) || Array.isArray(itemsValue);
        if (!itemsValid)
            return false;
        const currenciesValue = is.currencies;
        const currenciesValid = currenciesValue === undefined ||
        ...
        if ('medicinePouchState' in record && record.medicinePouchState !== undefined) {
            const pouch = record.medicinePouchState;
        ...
        if (typeof cs.autoAttack !== 'boolean' || typeof cs.autoCombatAI !== 'boolean')
            return false;
        ...
        if ('zoneState' in record && record.zoneState) {
            ...
            if (!Array.isArray(zs.unlockedZones) || typeof zs.zoneProgress !== 'object')
                return false;
        }
        if ('cityState' in record && record.cityState) {
            const cs = record.cityState;
            const invalid = (cs.currentCityId !== null && typeof cs.currentCityId !== 'string') ||
                !Array.isArray(cs.unlockedCityIds) ||
        ...
        if ('bountyState' in record && record.bountyState) {
            const bs = record.bountyState;
            if (typeof bs.activeByCityId !== 'object' || bs.activeByCityId === null)
                return false;
            if (typeof bs.lastRefreshAtByCityId !== 'object' || bs.lastRefreshAtByCityId === null)
                return false;
            if ('trackedByCityId' in bs && bs.trackedByCityId !== undefined) {
                if (typeof bs.trackedByCityId !== 'object' || bs.trackedByCityId === null)
                    return false;
                const values = Object.values(bs.trackedByCityId);
                if (!values.every((value) => value === null || typeof value === 'string'))
                    return false;
            }
        }
        if ('expeditionState' in record && record.expeditionState) {
            const es = record.expeditionState;
            if (typeof es.slots !== 'number')
                return false;
            if (!Array.isArray(es.active))
                return false;
        }
        if ('heartLawState' in record && record.heartLawState) {
            const hs = record.heartLawState;
            if (hs.selectedHeartLawId !== null && hs.selectedHeartLawId !== undefined && typeof hs.selectedHeartLawId !== 'string') {
                return false;
            }
            if (typeof hs.chapter !== 'number')
                return false;
            if (typeof hs.comprehension !== 'number')
                return false;
            if (!Array.isArray(hs.unlockedHeartLawIds))
                return false;
            if ('breathMode' in hs &&
                hs.breathMode !== undefined &&
                !['balanced', 'safe', 'fast'].includes(hs.breathMode)) {
                return false;
            }
            if ('studyEnabled' in hs && hs.studyEnabled !== undefined && typeof hs.studyEnabled !== 'boolean') {
                return false;
            }
            if ('studyTechniqueId' in hs &&
                hs.studyTechniqueId !== null &&
                hs.studyTechniqueId !== undefined &&
                typeof hs.studyTechniqueId !== 'string') {
                return false;
            }
            if ('lastInsightAt' in hs &&
                hs.lastInsightAt !== null &&
                hs.lastInsightAt !== undefined &&
                typeof hs.lastInsightAt !== 'number') {
                return false;
            }
            if ('nextInsightAt' in hs &&
        ...
        if ('manualPavilionState' in record && record.manualPavilionState) {
            const mps = record.manualPavilionState;
            if (!mps.stockByPavilionId || typeof mps.stockByPavilionId !== 'object')
                return false;
        }
        if ('manualSatchelState' in record && record.manualSatchelState) {
            const mss = record.manualSatchelState;
            if (!Array.isArray(mss.manuals))
                return false;
        }
        if ('techniqueState' in record && record.techniqueState) {
            const ts = record.techniqueState;
            if (typeof ts.selectedLoadoutId !== 'string')
                return false;
            if (!Array.isArray(ts.loadouts))
                return false;
            const loadouts = ts.loadouts;
            for (const loadout of loadouts) {
                ...
                if (typeof typed.id !== 'string' || typeof typed.name !== 'string')
                    return false;
                if (typeof typed.aiProfile !== 'string')
                    return false;
                const slots = typed.slots;
                if (!slots || typeof slots !== 'object')
                    return false;
                if (!Array.isArray(slots.active) || !Array.isArray(slots.passive))
                    return false;
        ...
        if ('trialState' in record && record.trialState) {
            const ts = record.trialState;
            if (typeof ts.progressByTrialId !== 'object' || ts.progressByTrialId === null)
                return false;
            const progressById = ts.progressByTrialId;
        ...
        if ('ruinsState' in record && record.ruinsState) {
            const rs = record.ruinsState;
            if (typeof rs.progressByRuinId !== 'object' || rs.progressByRuinId === null)
                return false;
            if ('autoRepeatDefault' in rs &&
                rs.autoRepeatDefault !== undefined &&
                typeof rs.autoRepeatDefault !== 'boolean') {
                return false;
            }
            if ('autoRestart' in rs &&
                rs.autoRestart !== undefined &&
                typeof rs.autoRestart !== 'boolean') {
                return false;
            }
            if ('runHistory' in rs && rs.runHistory !== undefined) {
                if (!Array.isArray(rs.runHistory))
                    return false;
                if (!rs.runHistory.every((entry) => isValidRuinsRunSummary(entry)))
                    return false;
            }
            if ('lastRunSummary' in rs && rs.lastRunSummary != null && !isValidRuinsRunSummary(rs.lastRunSummary)) {
                return false;
            }
            const progressById = rs.progressByRuinId;
            for (const value of Object.values(progressById)) {
                if (!value || typeof value !== 'object')
                    return false;
                const progress = value;
        ...
        if ('shopState' in record && record.shopState) {
            const ss = record.shopState;
            if (typeof ss.dayKey !== 'string')
                return false;
            if (typeof ss.purchasedToday !== 'object' || ss.purchasedToday === null)
                return false;
        }
        if ('equipmentState' in record && record.equipmentState) {
            const es = record.equipmentState;
            const weaponId = es.equippedWeaponId;
            const accessoryId = es.equippedAccessoryId;
            if (weaponId !== null && weaponId !== undefined && typeof weaponId !== 'string')
                return false;
            if (accessoryId !== null && accessoryId !== undefined && typeof accessoryId !== 'string')
                return false;
            const refine = es.refineLevelBySlot;
            if (refine) {
                if (typeof refine.weapon !== 'number' || typeof refine.accessory !== 'number')
                    return false;
            }
        }
        if ('buffState' in record && record.buffState) {
            const bs = record.buffState;
            if (!Array.isArray(bs.activeTalismans))
                return false;
            const active = bs.activeTalismans;
            for (const entry of active) {
                ...
                if (typeof entry.startedAt !== 'number')
                    return false;
                if (typeof entry.endsAt !== 'number')
                    return false;
                if ('bonuses' in entry && entry.bonuses !== undefined && typeof entry.bonuses !== 'object')
                    return false;
            }
        }
        if ('professionState' in record && record.professionState) {
            const ps = record.professionState;
            if ('alchemyQueue' in ps && !Array.isArray(ps.alchemyQueue))
                return false;
            if ('talismanQueue' in ps && !Array.isArray(ps.talismanQueue))
                return false;
            if ('forgeQueue' in ps && !Array.isArray(ps.forgeQueue))
                return false;
            if ('lastTickAt' in ps && typeof ps.lastTickAt !== 'number')
                return false;
            const queue = ps.alchemyQueue ?? [];
            for (const job of queue) {
                if (!job || typeof job !== 'object')
                    return false;
                if (typeof job.id !== 'string')
                    return false;
                if (typeof job.recipeId !== 'string')
                    return false;
                if (typeof job.qty !== 'number')
                    return false;
                if (typeof job.startedAt !== 'number')
                    return false;
                if (typeof job.endsAt !== 'number')
                    return false;
                if ('cityId' in job && job.cityId !== undefined && typeof job.cityId !== 'string')
                    return false;
            }
            const talismanQueue = ps.talismanQueue ?? [];
            ...
            const forgeQueue = ps.forgeQueue ?? [];
            for (const job of forgeQueue) {
                ...
                if ('targetSlot' in job && job.targetSlot !== undefined) {
                    if (job.targetSlot !== 'weapon' && job.targetSlot !== 'accessory')
                        return false;
                }
                if ('mode' in job && job.mode !== undefined) {
                    if (job.mode !== 'IDLE' && job.mode !== 'ASSISTED' && job.mode !== 'HANDS_ON')
                        return false;
                }
                ...
                if ('sessionId' in job && job.sessionId !== undefined && typeof job.sessionId !== 'string')
                    return false;
                if ('performance' in job && job.performance !== undefined) {
                    if (typeof job.performance !== 'object' || job.performance === null)
                        return false;
                    const perf = job.performance;
                    const checkNum = (value) => value === undefined || (typeof value === 'number' && Number.isFinite(value));
                    if (!checkNum(perf.heatScore))
                        return false;
                    if (!checkNum(perf.hammerScore))
                        return false;
                    if (!checkNum(perf.specialScore))
                        return false;
                    if (!checkNum(perf.qualityScore))
                        return false;
                    if ('stepBreakdown' in perf && perf.stepBreakdown !== undefined) {
                        if (!Array.isArray(perf.stepBreakdown))
                            return false;
                ...
                if ('resultSnapshot' in job && job.resultSnapshot !== undefined) {
                    if (typeof job.resultSnapshot !== 'object' || job.resultSnapshot === null)
                        return false;
                    const snapshot = job.resultSnapshot;
                    if (snapshot.outputItemId !== undefined && typeof snapshot.outputItemId !== 'string')
                        return false;
                    if (snapshot.outputBundle !== undefined) {
                        if (typeof snapshot.outputBundle !== 'object' || snapshot.outputBundle === null)
                            return false;
                        const bundle = snapshot.outputBundle;
                    ...
                    if (snapshot.beforeItem !== undefined && typeof snapshot.beforeItem !== 'object')
                        return false;
                    if (snapshot.afterItem !== undefined && typeof snapshot.afterItem !== 'object')
                        return false;
                }
                if ('cityId' in job && job.cityId !== undefined && typeof job.cityId !== 'string')
                    return false;
...
function decryptSaveData(encrypted) {
    try {
        ...
        pendingOfflineContext = buildOfflineContext(migrated.meta?.lastActiveAtMs ??
...
let pendingOfflineContext = null;
export function consumeOfflineContext() {
    const context = pendingOfflineContext;
    pendingOfflineContext = null;
    return context;
}
export function getLastLoadedSaveSnapshot() {
    return lastLoadedSaveData;
}
export function getLastLoadMigrationReport() {
    return lastLoadMigrationReport;
}
export function getLastLoadFailure() {
...
function applySaveData(saveData) {
    try {
        ...
        const cityState = saveData.cityState ??
            defaults.cityState ?? {
            currentCityId: null,
            unlockedCityIds: [],
        ...
        const collectionState = saveData.techCollectionState ?? defaults.techCollectionState ?? { unlockedTechs: {}, fragments: {}, rngSeed: undefined };
        const activityState = saveData.activityState ?? defaults.activityState ?? { active: null, lastChangedAt: null, history: [] };
        const savedActivity = activityState.active ?? null;
        const sanitizedActivity = savedActivity && COMBAT_ACTIVITY_TYPES.includes(savedActivity.type)
            ? null
            : savedActivity;
        const outskirtsState = saveData.outskirtsState ?? defaults.outskirtsState ?? { progressByOutskirtsId: {}, autoContinue: true, stopAtBoss: false };
        ...
        const bountyState = saveData.bountyState ?? defaults.bountyState ?? { activeByCityId: {}, lastRefreshAtByCityId: {}, trackedByCityId: {} };
        const expeditionState = saveData.expeditionState ?? defaults.expeditionState ?? { slots: 0, active: [], rareProgressByKey: {} };
        const ruinsState = saveData.ruinsState ??
        ...
        if (saveData.prestigeState) {
            ...
            usePrestigeStore.setState((state) => {
                ...
                const legacyUpgrades = prestigeState.upgrades;
        ...
        useInventoryStore.setState((state) => {
            const savedItems = saveData.inventoryState.items;
            const nextItems = {};
            ...
            const savedCurrencies = saveData.inventoryState.currencies;
            const gold = typeof savedCurrencies?.gold === 'string'
                ? savedCurrencies.gold
                : typeof saveData.inventoryState.gold === 'string'
                    ? saveData.inventoryState.gold
                    : '0';
            const spiritStones = typeof savedCurrencies?.spiritStones === 'string'
                ? savedCurrencies.spiritStones
                : typeof saveData.inventoryState.spiritStones === 'string'
                    ? saveData.inventoryState.spiritStones
                    : '0';
            const merit = typeof savedCurrencies?.merit === 'string'
                ? savedCurrencies.merit
                : typeof saveData.inventoryState.merit === 'string'
                    ? saveData.inventoryState.merit
                    : '0';
            state.currencies = {
                gold,
                spiritStones,
        ...
        const restoredTrials = Object.fromEntries(Object.entries(trialState.progressByTrialId ?? {}).map(([trialId, progress]) => [
            trialId,
            normalizeTrialProgress(progress),
        ]));
        ...
        useExpeditionStore.getState().hydrate({
            ...
            active: Array.isArray(expeditionState.active)
                ? expeditionState.active.map((run) => ({
                    ...run,
                    seed: typeof run.seed === 'number' && Number.isFinite(run.seed)
                        ? run.seed >>> 0
                        : (run.startedAt ?? Date.now()) >>> 0,
                }))
                : [],
            rareProgressByKey: { ...(expeditionState.rareProgressByKey ?? {}) },
        }, Date.now());
        useHeartLawStore.setState({
            ...
            breathMode: heartLawState.breathMode === 'safe' || heartLawState.breathMode === 'fast'
                ? heartLawState.breathMode
                : 'balanced',
            studyEnabled: typeof heartLawState.studyEnabled === 'boolean' ? heartLawState.studyEnabled : false,
            studyTechniqueId: typeof heartLawState.studyTechniqueId === 'string' || heartLawState.studyTechniqueId === null
                ? heartLawState.studyTechniqueId
                : null,
            lastInsightAt: typeof heartLawState.lastInsightAt === 'number' && Number.isFinite(heartLawState.lastInsightAt)
                ? heartLawState.lastInsightAt
                : null,
            nextInsightAt: typeof heartLawState.nextInsightAt === 'number' && Number.isFinite(heartLawState.nextInsightAt)
                ? heartLawState.nextInsightAt
                : null,
            insight: heartLawState.insight ?? null,
            stability: typeof heartLawState.stability === 'number' && Number.isFinite(heartLawState.stability)
                ? heartLawState.stability
                : 0,
            stabilityCap: typeof heartLawState.stabilityCap === 'number' && Number.isFinite(heartLawState.stabilityCap)
                ? heartLawState.stabilityCap
                : 100,
            activeCultivationConsumables: Array.isArray(heartLawState.activeCultivationConsumables)
                ? heartLawState.activeCultivationConsumables
                    .filter((entry) => entry && typeof entry.itemId === 'string' && typeof entry.family === 'string')
                    .map((entry) => ({
                    itemId: entry.itemId,
                    family: entry.family,
                ...
                : [],
            insightProgressMs: typeof heartLawState.insightProgressMs === 'number' && Number.isFinite(heartLawState.insightProgressMs)
                ? heartLawState.insightProgressMs
                : 0,
            insightTargetMs: typeof heartLawState.insightTargetMs === 'number' && Number.isFinite(heartLawState.insightTargetMs)
                ? heartLawState.insightTargetMs
                : null,
        });
        ...
        const hydratedRuinProgress = Object.fromEntries(Object.entries(ruinsState.progressByRuinId ?? {}).map(([ruinId, progress]) => [
            ruinId,
        ...
        useEquipmentStore.setState({
            ...
            temperBonusesBySlot: {
                weapon: Array.isArray(equipmentState.temperBonusesBySlot?.weapon)
                    ? (equipmentState.temperBonusesBySlot?.weapon ?? [])
                    : [],
                accessory: Array.isArray(equipmentState.temperBonusesBySlot?.accessory)
                    ? (equipmentState.temperBonusesBySlot?.accessory ?? [])
                    : [],
            },
            forgeToolTiers: {
                anvil: Math.max(1, Math.min(10, Number(equipmentState.forgeToolTiers?.anvil) || 1)),
                hammer: Math.max(1, Math.min(10, Number(equipmentState.forgeToolTiers?.hammer) || 1)),
                bellows: Math.max(1, Math.min(10, Number(equipmentState.forgeToolTiers?.bellows) || 1)),
                quenchTub: Math.max(1, Math.min(10, Number(equipmentState.forgeToolTiers?.quenchTub) || 1)),
            },
        });
        ...
        const fallbackCityId = useCityStore.getState().currentCityId ??
 ...
 */
export function deleteSaveAndHardReset() {
    try {
        localStorage.removeItem(SAVE_KEY);
        localStorage.removeItem(BACKUP_A_KEY);
        localStorage.removeItem(BACKUP_B_KEY);
