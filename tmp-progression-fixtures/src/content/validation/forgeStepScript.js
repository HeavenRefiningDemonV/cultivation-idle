const HEAT_STEPS = new Set(['HEAT_TO', 'HEAT_MATERIAL']);
const STRIKE_STEPS = new Set(['HAMMER_PATTERN']);
const SPECIAL_STEPS = new Set(['ENGRAVE_RUNE', 'LAY_FORMATION', 'QUENCH', 'TEMPER']);
const RING_QTE_STEPS = new Set(['HAMMER_PATTERN', 'ENGRAVE_RUNE', 'LAY_FORMATION']);
const FINISH_STEPS = new Set(['FINISH']);
...
export function validateForgeStepScript(stepScript) {
    const errors = [];
    if (!stepScript || stepScript.length === 0) {
        return { ok: false, errors: ['Forge step script is empty.'] };
    }
    const expectedSteps = ['HEAT', 'STRIKE', 'HEAT', 'STRIKE', 'HEAT'];
    ...
    stepScript.forEach((step, index) => {
        ...
        if (RING_QTE_STEPS.has(step.type)) {
            const patternId = step.patternId;
...
export function isForgeStepScriptValid(stepScript) {
