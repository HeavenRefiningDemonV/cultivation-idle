...
export type ConsumableDomain = 'combat' | 'cultivation';
...
export type CombatConsumableEffect =
  ...
  | { kind: 'cleanseOrFallbackBuff'; fallbackStat: 'def'; fallbackValue: number; durationSec: number };
...
export interface ConsumableSpec {
  itemId: string;
  domain: ConsumableDomain;
  usage: ConsumableUsage;
  cooldownSec: number;
  recommendedSlot: 'healing' | 'utility' | 'specialty' | null;
  effect: CombatConsumableEffect | CultivationConsumableEffect;
  family?: CultivationConsumableFamily;
  durationSec?: number;
  shortLabel: string;
  longLabel?: string;
...
export function getConsumableSpec(itemId: string): ConsumableSpec | null { return CONSUMABLE_SPECS[itemId] ?? null; }
export function isCombatUsableConsumable(itemId: string): boolean { const spec = getConsumableSpec(itemId); return !!spec && spec.domain === 'combat'; }
export function isCultivationUsableConsumable(itemId: string): boolean { const spec = getConsumableSpec(itemId); return !!spec && spec.domain === 'cultivation'; }
