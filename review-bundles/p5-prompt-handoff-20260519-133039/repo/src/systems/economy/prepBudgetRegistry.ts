...
import { GATE_PREP_PACKAGE_CATALOG, type GatePrepSupplementLane } from '../../features/apothecary/gatePrepPackageCatalog.js';
import {
  ECONOMIC_TRANSITION_META,
  type EconomicForgeFloorTarget,
  type EconomicGateIndex,
  type EconomicPrepBackgroundExpectation,
  type EconomicTransitionId,
  type EconomicStockLine,
  type EconomicSupplementLane,
  type GoldSpendRange,
} from './economicConstants.js';

export interface EconomicPrepBudgetPackage {
  forgeFloor: EconomicForgeFloorTarget;
  stockPackage: {
    directCore: readonly EconomicStockLine[];
    supplementLanes: readonly EconomicSupplementLane[];
  };
  backgroundExpectations: readonly EconomicPrepBackgroundExpectation[];
  goldSpendRange: GoldSpendRange;
}

export interface EconomicPrepBudgetRegistryEntry {
  ...
  cityId: CityId;
  cityIndex: number;
  minimumPrepPackage: EconomicPrepBudgetPackage;
  recommendedPrepPackage: EconomicPrepBudgetPackage;
}

function cloneSupplementLane(lane: GatePrepSupplementLane): EconomicSupplementLane {
  return {
    key: lane.key,
    label: lane.label,
    qty: lane.qty,
...
function getRecommendedPackageByTransitionId(transitionId: EconomicTransitionId) {
...
export const PREP_BUDGET_REGISTRY: readonly EconomicPrepBudgetRegistryEntry[] = [
  {
...
] as const satisfies readonly EconomicPrepBudgetRegistryEntry[];

export const PREP_BUDGET_REGISTRY_BY_GATE_INDEX = Object.fromEntries(
  PREP_BUDGET_REGISTRY.map((entry) => [entry.gateIndex, entry]),
) as Record<EconomicGateIndex, EconomicPrepBudgetRegistryEntry>;

export const PREP_BUDGET_REGISTRY_BY_TRANSITION_ID = Object.fromEntries(
  PREP_BUDGET_REGISTRY.map((entry) => [entry.transitionId, entry]),
) as Record<EconomicTransitionId, EconomicPrepBudgetRegistryEntry>;

export const PREP_BUDGET_REGISTRY_BY_FROM_REALM = Object.fromEntries(
  PREP_BUDGET_REGISTRY.map((entry) => [entry.fromRealmId, entry]),
) as Record<MajorRealmId, EconomicPrepBudgetRegistryEntry>;

export const PREP_BUDGET_REGISTRY_BY_CITY_ID = Object.fromEntries(
  PREP_BUDGET_REGISTRY.map((entry) => [entry.cityId, entry]),
) as Record<CityId, EconomicPrepBudgetRegistryEntry>;

export function getAllPrepBudgetRegistryEntries(): EconomicPrepBudgetRegistryEntry[] {
  return [...PREP_BUDGET_REGISTRY];
}

export function getPrepBudgetByGateIndex(gateIndex: number): EconomicPrepBudgetRegistryEntry | null {
  return PREP_BUDGET_REGISTRY_BY_GATE_INDEX[gateIndex as EconomicGateIndex] ?? null;
}

export function getPrepBudgetByTransitionId(transitionId: EconomicTransitionId | null | undefined): EconomicPrepBudgetRegistryEntry | null {
  if (!transitionId) return null;
  return PREP_BUDGET_REGISTRY_BY_TRANSITION_ID[transitionId] ?? null;
}

export function getPrepBudgetByCurrentRealmId(realmId: MajorRealmId | null | undefined): EconomicPrepBudgetRegistryEntry | null {
  if (!realmId) return null;
  return PREP_BUDGET_REGISTRY_BY_FROM_REALM[realmId] ?? null;
}

export function getPrepBudgetByNextCityId(cityId: CityId | null | undefined): EconomicPrepBudgetRegistryEntry | null {
