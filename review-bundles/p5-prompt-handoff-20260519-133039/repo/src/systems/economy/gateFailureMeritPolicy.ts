...
import { clampSupportReserveGateIndex, type SupportReserveGateIndex } from './supportCurrencyTargets.js';

export interface GateFailureMeritPolicy {
...
const GATE_FAILURE_MERIT_POLICY: Record<SupportReserveGateIndex, GateFailureMeritPolicy> = {
...
export function getGateFailureMeritPolicyByGateIndex(gateIndex: number): GateFailureMeritPolicy {
  return GATE_FAILURE_MERIT_POLICY[clampSupportReserveGateIndex(gateIndex)];
}

export function getAllGateFailureMeritPolicies(): GateFailureMeritPolicy[] {
  return Object.values(GATE_FAILURE_MERIT_POLICY);
}

export function resolveGateIndexForTrial(content: ValidatedContent | null | undefined, trial: TrialDef | null | undefined): SupportReserveGateIndex {
  if (content && trial) {
    try {
...
export function getGateFailureMeritPolicyForTrial(content: ValidatedContent | null | undefined, trial: TrialDef | null | undefined): GateFailureMeritPolicy {
  return getGateFailureMeritPolicyByGateIndex(resolveGateIndexForTrial(content, trial));
}

export function projectMeritAfterEligibleDefeats(currentMerit: string, gateIndex: number, eligibleDefeatCount: number): string {
