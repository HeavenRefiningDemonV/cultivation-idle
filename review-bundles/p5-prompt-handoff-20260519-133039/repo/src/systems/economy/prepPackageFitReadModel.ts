...
export type PrepCoverageMode = 'direct' | 'brew' | 'hybrid' | 'unsupported';

export interface PrepPackageFitReport {
  gateIndex: number;
  transitionId: string;
  ...
  directCore: Array<{
    itemId: string;
    targetQty: number;
    soldByCityShop: boolean;
    directDailyLimit: number | null;
    directUnitPrice: number | null;
    convenienceShareOfDailyLimit: number | null;
    coverageMode: PrepCoverageMode;
    visibleLiveBrewSupport: boolean;
    visibleLiveSourceIds: string[];
    sourceFamilies: string[];
    blocker: string | null;
  }>;
  supplementLanes: Array<{
    key: string;
    supportedOptions: string[];
    honestOptions: string[];
    hasAtLeastOneHonestOption: boolean;
    routeNotes: string[];
  }>;
  forgeFit: {
    targetWeaponRefine: number;
    targetAccessoryRefine: number;
    targetTemperSuccesses: number;
    targetRuneRecommendation: { minimum: number; recommendedLow: number; recommendedHigh: number; note: string };
    forgeVisibleInCity: boolean;
    requiredVisibleInputs: string[];
    supportsTarget: boolean;
  };
  backgroundExpectationFit: {
    expectations: Array<{ key: string; qty: number; routeFamily: string; plausible: boolean; dependsOnFutureOnly: boolean }>;
    plausible: boolean;
  };
  goldBudgetFit: {
    directShelfSpendEstimate: number;
    brewBackedSpendEstimate: number;
    forgeRelatedSpendEstimate: number;
    totalHonestPrepSpendEstimate: number;
    minimumRange: number;
    recommendedRange: number;
    fitsMinimumRange: boolean;
    fitsRecommendedRange: boolean;
  };
  sourceRealism: {
    noNonLiveConsumableDependencies: boolean;
    noPostGateOnlyDependencies: boolean;
  };
  honest: boolean;
  blockers: string[];
  warnings: string[];
  dependencySummary: {
    ...
    directTotal: number;
    supplementCovered: number;
    supplementTotal: number;
  };
}

function toRouteFamily(expectationKey: string) {
  if (expectationKey.includes('bounty')) return 'bounties';
  if (expectationKey.includes('ruins')) return 'ruins';
  if (expectationKey.includes('expedition')) return 'expeditions';
  return 'unknown';
}

export function buildPrepPackageFitReport(content: ValidatedContent, gateIndex: number): PrepPackageFitReport {
  const entry = getAllPrepBudgetRegistryEntries().find((candidate) => candidate.gateIndex === gateIndex);
  if (!entry) throw new Error(`[PrepPackageFitReadModel] Missing prep budget for gate ${gateIndex}`);
  ...
  const directCore = coverage.lineCoverage.map((line) => {
    ...
    const coverageMode: PrepCoverageMode = line.soldDirectly && line.visibleLiveBrew
      ? 'hybrid'
      : line.soldDirectly
        ? 'direct'
    ...
    if (!liveConsumables.has(line.itemId)) blockers.push(`non_live_consumable:${line.itemId}`);
  ...
  const supplementLanes = coverage.supplementCoverage.map((lane) => {
    ...
    if (!lane.honest) blockers.push(`supplement_lane_unsupported:${lane.key}`);
    return {
      key: lane.key,
  ...
  const forgeInputIds = [...new Set(getLiveCriticalBuildCorrectionItemIds().flatMap((itemId) =>
    listVisibleAlchemyInputsForOutput(content, itemId).map((input) => input.itemId),
  ))];
  ...
  const backgroundExpectationFit = {
    expectations: entry.recommendedPrepPackage.backgroundExpectations.map((expectation) => {
      ...
      if (!plausible) warnings.push(`background_expectation_high:${expectation.key}`);
  ...
  const forgeRelatedSpendEstimate = (
    entry.recommendedPrepPackage.forgeFloor.weaponRefine
    + entry.recommendedPrepPackage.forgeFloor.accessoryRefine
    + entry.recommendedPrepPackage.forgeFloor.temperSuccesses
  ) * 1200;
  ...
  if (!goldBudgetFit.fitsMinimumRange || !goldBudgetFit.fitsRecommendedRange) blockers.push('gold_budget_fit_failed');
  if (!forgeFit.supportsTarget) blockers.push('forge_fit_unsupported');
  if (!backgroundExpectationFit.plausible) warnings.push('background_expectation_stretch');
...
export function buildAllPrepPackageFitReports(content: ValidatedContent) {
