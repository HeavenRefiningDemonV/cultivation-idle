...
export type ActivityRewardRole = 'outskirts' | 'ruins';

export interface CityActivityRewardRoleProfile {
  cityId: string;
  cityIndex: number;
  commonFieldMaterialIds: string[];
  targetedMaterialIds: string[];
  deterministicAnchorItemId: string;
  ruinLeadMaterialIds: string[];
  outskirtsRareSpikeItemIds: string[];
...
] as const satisfies readonly CityActivityRewardRoleProfile[];

export const CITY_ACTIVITY_REWARD_ROLE_PROFILES = ROLE_PROFILES;

export const CITY_ACTIVITY_REWARD_ROLE_PROFILES_BY_ID = Object.fromEntries(
  ROLE_PROFILES.map((profile) => [profile.cityId, profile]),
) as Record<string, CityActivityRewardRoleProfile>;

export function listCityRewardRoleProfiles(): CityActivityRewardRoleProfile[] {
  return CITY_PACKAGE_REGISTRY.map((entry) => CITY_ACTIVITY_REWARD_ROLE_PROFILES_BY_ID[entry.cityId]).filter(
    (profile): profile is CityActivityRewardRoleProfile => Boolean(profile),
  );
}

export function getCityRewardRoleProfile(cityId: string): CityActivityRewardRoleProfile {
...
export function getDeterministicRuinAnchorItemId(cityId: string): string {
  return getCityRewardRoleProfile(cityId).deterministicAnchorItemId;
}

export function getRuinLeadMaterialIds(cityId: string): string[] {
  return [...getCityRewardRoleProfile(cityId).ruinLeadMaterialIds];
}

export function getTargetedCityMaterialIds(cityId: string): string[] {
  return [...getCityRewardRoleProfile(cityId).targetedMaterialIds];
}

export function getCommonFieldMaterialIds(cityId: string): string[] {
  return [...getCityRewardRoleProfile(cityId).commonFieldMaterialIds];
}

export function getOutskirtsRareSpikeItemIds(cityId: string): string[] {
  return [...getCityRewardRoleProfile(cityId).outskirtsRareSpikeItemIds];
}

export function getActivityRewardIntent(activity: ActivityRewardRole): string {
...
export function isDeterministicRuinAnchor(cityId: string, itemId: string): boolean {
  return getDeterministicRuinAnchorItemId(cityId) === itemId;
}

export function isTargetedCityMaterial(cityId: string, itemId: string): boolean {
  return getCityRewardRoleProfile(cityId).targetedMaterialIds.includes(itemId);
}

export function isCommonFieldMaterial(cityId: string, itemId: string): boolean {
  return getCityRewardRoleProfile(cityId).commonFieldMaterialIds.includes(itemId);
}

export function isOutskirtsRareSpike(cityId: string, itemId: string): boolean {
  return getCityRewardRoleProfile(cityId).outskirtsRareSpikeItemIds.includes(itemId);
}

export function isRuinLeadMaterial(cityId: string, itemId: string): boolean {
  return getCityRewardRoleProfile(cityId).ruinLeadMaterialIds.includes(itemId);
}

export function isRoleSupportItem(cityId: string, itemId: string): boolean {
  return getCityRewardRoleProfile(cityId).supportItemIds.includes(itemId);
}

export type RewardRoleClassification =
  | 'anchor'
  | 'targeted_local'
  | 'common_field'
  | 'support'
...
export function classifyActivityRewardItem(cityId: string, itemId: string): RewardRoleClassification {
  ...
  if (isRoleSupportItem(cityId, itemId)) return 'support';
  return 'other';
}
