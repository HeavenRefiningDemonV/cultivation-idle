...
export function getAlchemyTimeMultiplier(mastery: number): number {
  return mastery >= TIME_REDUCTION_THRESHOLD ? 0.9 : 1;
}

export function getIdleYieldMultiplierForMastery(mastery: number): number {
  return mastery >= YIELD_THRESHOLD ? IDLE_MAX_YIELD_MULTIPLIER : 1;
}

export function buildAlchemyOutputs(
