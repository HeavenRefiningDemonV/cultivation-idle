export type GatePrepTransitionId =
...
export interface GatePrepLineDef {
  itemId: string;
  qty: number;
}

export interface GatePrepSupplementLane {
  key: string;
  label: string;
  qty: number;
...
export interface GatePrepPackageDef {
  transitionId: GatePrepTransitionId;
  cityId: string;
  label: string;
  directCore: GatePrepLineDef[];
  supplementLanes: GatePrepSupplementLane[];
}

export const GATE_PREP_PACKAGE_CATALOG: GatePrepPackageDef[] = [
  {
...
export function getGatePrepPackageForCity(cityId: string | null | undefined) {
