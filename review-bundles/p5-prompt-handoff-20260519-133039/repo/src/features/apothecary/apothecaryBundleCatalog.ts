...
export interface ApothecaryBundleCatalogLine {
  itemId: string;
  targetQty: number;
}

export interface ApothecaryBundleCatalogEntry {
  cityId: string;
  nameSuffix: string;
  description: string;
  lines: ApothecaryBundleCatalogLine[];
}

export const APOTHECARY_BUNDLE_CATALOG: Record<string, ApothecaryBundleCatalogEntry> = {
...
export function getApothecaryBundleCatalogEntry(shop: ApothecaryShopDef | null | undefined) {
  if (!shop?.cityId) return null;
