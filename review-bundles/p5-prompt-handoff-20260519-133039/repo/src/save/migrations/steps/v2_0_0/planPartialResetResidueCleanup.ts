...
const DETECTION_FIELD_PATHS: Record<keyof ReturnType<typeof applyPartialResetResidueCleanup>['detected'], string> = {
...
const DETECTION_MESSAGES: Record<keyof typeof DETECTION_FIELD_PATHS, string> = {
...
export const v2_0_0_plan_partial_reset_residue_cleanup: MigrationStep = {
  ...
  run: (save) => {
    ...
    const detectedCategories = Object.entries(cleanup.detected)
      ...
      .map(([key]) => key as keyof typeof cleanup.detected);

    if (cleanup.didMutate) {
      warnings.push(
        warning('PARTIAL_RESET_RESIDUE_DETECTED', `Detected clean-life baseline with lingering reset residue in ${detectedCategories.join(', ')}.`, '1.7', 'warning', 'gameState.realm.index'),
        warning('CLEAN_NEW_LIFE_NORMALIZATION_PLAN_READY', 'Clean new-life normalization is ready for packet 1.7 and applies on migration apply mode.', '1.7', 'info', 'cityState'),
      );
    ...
    return createStepResult(
      ...
      cleanup.didMutate
        ...
        : 'No partial-reset residue detected.',
      { warnings, touchedFieldPaths: touched, plannedMutations, didMutate: cleanup.didMutate },
    );
  },
};
