...
type CityModuleSource = {
  id: string;
  index?: number;
  modules?: string[];
};

const DEFAULT_CITY_FLAGS: SaveCityState['cityFlagsById'][string] = {
...
const getDefaultModule = (modules: string[]): string | null => {
  if (modules.includes('outskirts')) return 'outskirts';
  return modules[0] ?? null;
};

const getCitySources = (content: ValidatedContent | null | undefined): CityModuleSource[] => {
...
const getUnlockedCityIdsForRealm = (
  content: ValidatedContent | null | undefined,
  realmId: MajorRealmId,
  existingUnlockedCityIds: string[],
): string[] => {
  if (!content) {
...
export const normalizeCitySaveState = ({
  content,
  realmIndex,
  cityState,
}: {
  ...
  const incomingUnlocked = Array.isArray(incoming.unlockedCityIds)
    ? incoming.unlockedCityIds.filter((cityId): cityId is string => typeof cityId === 'string' && validCityIds.has(cityId))
    : [];
  ...
  const unlockedCityIds =
    normalizedUnlockedCityIds.length > 0
  ...
  const cityFlagsById = Object.fromEntries(
    unlockedCityIds.map((cityId) => {
  ...
  ) as SaveCityState['cityFlagsById'];

  const highestUnlockedCityId = unlockedCityIds.at(-1) ?? citySources[0]?.id ?? null;
  const existingCurrentCityId =
    typeof incoming.currentCityId === 'string' && unlockedCityIds.includes(incoming.currentCityId)
  ...
  return {
    ...
    cityFlagsById:
      Object.keys(cityFlagsById).length > 0
        ? cityFlagsById
