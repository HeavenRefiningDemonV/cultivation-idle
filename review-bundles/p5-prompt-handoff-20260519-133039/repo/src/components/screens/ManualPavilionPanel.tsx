...
function BookSpineSlot({
  ...
  return (
    ...
    >
      <div className="pavilionSpineName">
        {technique?.name ?? slot.techniqueId}
      </div>
      <div className="pavilionSpineMeta" aria-label="Manual metadata">
        {tierIcon && (
          <span
            className="pavilionSpineMetaIcon"
            role="img"
...
export function ManualPavilionPanel({ pavilionId }: ManualPavilionPanelProps) {
  ...
  const pavilion = useContentStore((state) =>
    pavilionId ? state.maps.pavilionsById[pavilionId] : undefined,
  );
  const techniquesById = useContentStore((state) => state.maps.techniquesById);
  const manualSystem = useContentStore(
    (state) => state.raw?.economy?.manualSystem as any,
  );
  ...
  const stock = useManualPavilionStore((state) =>
    pavilionId ? state.stockByPavilionId[pavilionId] : null,
  );
  ...
  const canAffordCurrency = useInventoryStore(
    (state) => state.canAffordCurrency,
  );
  const openManualSatchel = useUIStore((state) => state.openManualSatchel);
  const setActiveTab = useUIStore((state) => state.setActiveTab);
  const requestTechniqueFocus = useUIStore(
    (state) => state.requestTechniqueFocus,
  );
  const addNotification = useUIStore((state) => state.addNotification);
  const satchelCount = useManualSatchelStore(
  ...
  const [purchaseResult, setPurchaseResult] = useState<
    (ManualPurchaseResult & { studied?: boolean }) | null
  >(null);
  const [purchaseError, setPurchaseError] = useState<string | null>(null);
  const [refreshFlash, setRefreshFlash] = useState(0);
  const [flashOn, setFlashOn] = useState(false);
  const [justPurchasedSlotId, setJustPurchasedSlotId] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const exists = stock.slots.some(
      (slot) => slot.slotIndex === selectedSlotId,
    );
    if (!exists) {
      setSelectedSlotId(stock.slots[0]?.slotIndex ?? null);
    }
  ...
  useEffect(() => {
    ...
    const exists = stock.slots.some(
      (slot) => slot.slotIndex === selectedManualSlotId,
    );
    if (!exists) {
  ...
  const gradeSold = normalizeGradeValue(
    pavilion?.gradeSold ??
      manualSystem?.pavilions?.manualPricesByCityIndex?.[
        pavilion?.cityIndex ?? 0
      ]?.grade,
  );
  const pityEpicMax =
    manualSystem?.pavilions?.pity?.featuredEpicPityToGuarantee ?? 10;
  const pityLegendaryMax =
    manualSystem?.pavilions?.pity?.featuredLegendaryPityToGuarantee ?? 30;

  const selectedSlot = useMemo(() => {
    const direct = stock?.slots.find(
      (slot) => slot.slotIndex === selectedSlotId,
    );
  ...
  const selectedManualSlot = useMemo(() => {
    ...
    return (
      stock.slots.find((slot) => slot.slotIndex === selectedManualSlotId) ??
      null
    );
  }, [selectedManualSlotId, stock]);
  ...
  const offerAnalysisBySlotIndex = useMemo(() => {
    const entries = new Map<number, ManualOfferAnalysis>();
    (stock?.slots ?? []).forEach((slot) => {
      entries.set(
        slot.slotIndex,
        analyzeManualOffer({
  ...
  const handleRefresh = () => {
    ...
    const remainingMs = stock
      ? Math.max(0, stock.nextRefreshAt - timestamp)
      : 0;
    ...
    if (stock && !ready) {
      ...
      addNotification(
        "info",
        `Restock not ready — ${remainingLabel} remaining.`,
        2500,
      );
      return;
    }
    ...
    const updatedStock =
      useManualPavilionStore.getState().stockByPavilionId[pavilionId];
    let message = "Pavilion restocked.";
    const latestHistory =
      updatedStock?.history?.[updatedStock.history.length - 1];
    if (latestHistory?.featured) {
      const featuredName =
        techniquesById[latestHistory.featured.techniqueId]?.name ??
        latestHistory.featured.techniqueId;
  ...
  const handlePurchase = (mode: "buy" | "buyAndStudy") => {
    ...
    setPurchaseResult(null);
    setPurchaseError(null);
    const result = buyManual({
      pavilionId,
      stockId: selectedManualSlot.slotIndex,
      mode,
    });
    if (!result.ok) {
      ...
      addNotification(
        "error",
        friendly ? `Purchase failed: ${friendly}` : "Purchase failed.",
        3000,
      );
      setPurchaseResult(null);
      return;
    ...
    if (mode === "buyAndStudy" && result.outcome === "manualGranted") {
      if (result.manualInstanceId) {
        const studyResult = useManualSatchelStore
          .getState()
          .startStudy(result.manualInstanceId);
        if (studyResult.ok) {
        ...
        addNotification(
          "info",
          "Purchased, but could not start study (missing manual instance).",
          3000,
        );
      }
    }
    ...
    if (finalResult.outcome === "manualGranted") {
      const manualName =
        (selectedManualSlot
          ? techniquesById[selectedManualSlot.techniqueId]?.name
          : undefined) ??
      ...
      addNotification(
        "info",
        `Duplicate converted (+${finalResult.fragmentsGained} fragments).`,
        2500,
      );
    }
    ...
    purchasePulseTimeoutRef.current = window.setTimeout(
      () => setJustPurchasedSlotId(null),
      700,
    );
  };
          ...
          <div className={"pavilionShelfRowSpines"} role="list">
            {spineEntries.map((entry) => (
              <BookSpineSlot
                ...
                onSelect={(event) =>
                  entry.slot && handleSelect(entry.slot, event)
                }
      ...
      <div className={"pavilionHistory"}>
        ...
        {historyOpen && (
          <div className={"pavilionHistoryBody"}>
            {entries.length === 0 && (
              <div className={"pavilionHistoryLine"}>No refreshes yet.</div>
            )}
            {entries.map((entry, idx) => {
              const featuredName = entry.featured
                ? (techniquesById[entry.featured.techniqueId]?.name ??
                  entry.featured.techniqueId)
                : "—";
              const rareSummary = entry.rares
                ?.map(
                  (rare) =>
                    techniquesById[rare.techniqueId]?.name ?? rare.techniqueId,
                )
                .join(", ");
              ...
              const relativeLabel =
                agoMinutes >= 1 ? `${agoMinutes}m ago` : "just now";
              return (
                <div key={idx} className={"pavilionHistoryLine"}>
                  <div>
                    ...
                    <div className={"pavilionHistoryEntry"}>
                      ...
                      {entry.featured
                        ? `(${rarityLabel(entry.featured.rarity)})`
                        : ""}
                    </div>
      ...
      <div className={"pavilionRefreshBar pavilionRefreshBar--inline"}>
        <div className={"pavilionRefreshMeta"}>
          <div className={"pavilionRefreshLine"}>
            <span>
              Next refresh: {ready ? "Ready" : formatDurationHMS(remaining)}
            </span>
  ...
  if (!isContentLoaded) {
    return (
      <div className={"worldScreenPlaceholder"}>
        ...
        <div className={"worldScreenPlaceholderBody"}>
          {isContentLoading
            ? "Loading manuals and pavilion data."
  ...
  if (!pavilion) {
    return (
      <div className={"worldScreenPlaceholder"}>
        <div className={"worldScreenPlaceholderHeader"}>
          <div className={"worldScreenPlaceholderTitle"}>
            Pavilion data missing
          </div>
          <div className={"worldScreenPlaceholderKey"}>{pavilionId}</div>
        </div>
        <div className={"worldScreenPlaceholderBody"}>
          Pavilion definition not found.
        </div>
      </div>
    );
  }
  ...
  if (!stock) {
    return (
      <div className={"worldScreenPlaceholder"}>
        <div className={"worldScreenPlaceholderHeader"}>
          <div className={"worldScreenPlaceholderTitle"}>
            Loading pavilion stock…
          </div>
          <div className={"worldScreenPlaceholderKey"}>{pavilionId}</div>
        </div>
        <div className={"worldScreenPlaceholderBody"}>
          Generating visible stock for this pavilion.
        </div>
      </div>
    );
  }
  ...
  const hoveredSlot = hovered
    ? stock.slots.find((slot) => slot.slotIndex === hovered.slotIndex)
    : undefined;
  const hoveredOfferAnalysis = hoveredSlot
    ? (offerAnalysisBySlotIndex.get(hoveredSlot.slotIndex) ?? null)
    : null;
  const hoveredTechnique = hoveredSlot
    ? techniquesById[hoveredSlot.techniqueId]
    : undefined;
  ...
  const hoveredState = hoveredSlot
    ? resolveSpineState(hoveredSlot)
    : "placeholder";
  const tooltipAnchorLeft = hovered
  ...
  const computedTooltipLeft = Math.min(
    maxTooltipLeft,
    Math.max(12, tooltipAnchorLeft),
  );
  ...
  const tooltipTop = hovered
    ? shouldFlipTooltip
      ...
      : hovered.rect.top - 10
    : 0;
  const tooltipTransform = shouldFlipTooltip
    ? "translate(-50%, 0)"
    : "translate(-50%, -100%)";
  const hoveredPriceLine = hoveredSlot
    ...
    : "";
  const hoveredStudyDurationLabel = hoveredSlot
    ? getStudyDurationLabelByGrade(hoveredSlot.grade)
    : null;
  const hoveredDuplicateFragmentValue = hoveredSlot
  ...
  return (
    <div className={"manualPavilionPanel manualPavilionPanel--v2"}>
      <PaperCard variant="tray" className={"pavilionTopRibbon"}>
        <div className={"pavilionTopLeft"}>
          ...
          <div className={"pavilionSubtitle"}>
            Grade sold: {gradeLabel(gradeSold as ManualGrade)} • Grade cap:{" "}
            {gradeLabel(gradeCap)}
          </div>
        ...
        <div className={"pavilionTopRight"}>
          <button
            className={"worldScreenModuleButton pavilionSatchelButton"}
      ...
      <ManualDetailModal
        ...
        offerAnalysis={selectedOfferAnalysis}
        studyDurationLabel={selectedStudyDurationLabel}
        duplicateFragmentValue={selectedDuplicateFragmentValue}
      />
      {hovered && hoveredSlot && (
        <div className="pavilionSpineTooltipLayer" aria-hidden="true">
          ...
          >
            ...
            {hoveredState !== "available" && (
              <div className="pavilionSpineTooltipLine">
                Status: {hoveredState}
              </div>
            )}
            <div className="pavilionSpineTooltipMicro">
              Buy → Satchel → Study → Techniques
            </div>
          </div>
        </div>
      )}
