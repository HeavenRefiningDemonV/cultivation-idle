...
const OUT_ROOT = 'docs/release/qa/ui-cutover/outskirts-exact/p0-freeze';
const RAW_EVIDENCE_FOLDER = 'docs/release/qa/ui-cutover/phase-6-combat-preflight/01-outskirts';
const CAPTURE_ATTEMPT_PATH = `${OUT_ROOT}/outskirtsExactP0CaptureAttempt.json`;
...
interface OutskirtsExactP0BaselineReport {
  schemaVersion: 'outskirts-exact-p0-baseline.v1';
  generatedAt: string;
  packetId: 'P0';
...
function readCaptureAttempt(rootDir: string): CaptureAttemptRecord | null {
  ...
  if (parsed?.schemaVersion !== 'outskirts-exact-p0-capture-attempt.v1') return null;
  return parsed;
}
...
function firstMeaningfulLine(value: string): string | null {
  ...
  const preferred =
    ...
    ?? lines.find((entry) => entry.includes('[outskirts-exact-p0-capture] capture failed.'))
...
function buildOutskirtsExactP0Baseline(rootDir: string): OutskirtsExactP0BaselineReport {
  const phase6 = buildPhase6CombatPreflightReport(rootDir);
  const coverage = phase6.screenshotCoverage.find((entry) => entry.surfaceId === 'outskirts');
  if (!coverage) {
    throw new Error('Outskirts screenshot coverage was not found in phase-6 preflight report builder output.');
  }

  const audit = auditPhase6CombatEvidence(rootDir, ['outskirts']);
  ...
  return {
    ...
    packetId: 'P0',
    packetObjective: 'Freeze and evidence-capture the current live Outskirts surface so later exact-mockup work can diff against a deterministic baseline.',
    whyNow: 'Outskirts replacement work is expected to move from combat-shell composition to exact-mockup composition; this packet freezes current truth before that change starts.',
    dependencies: [
    ...
    regenerationCommands: {
      capture: 'npm run release:outskirts-exact-p0:capture',
      audit: 'npm run release:outskirts-exact-p0:audit',
      report: 'npm run release:outskirts-exact-p0:report',
...
function toMarkdown(report: OutskirtsExactP0BaselineReport): string {
  const lines: string[] = [];
  lines.push('# Outskirts Exact Mockup — P0 Freeze Baseline');
  lines.push('');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const report = buildOutskirtsExactP0Baseline(rootDir);
  if (write) {
    const outRoot = path.resolve(rootDir, OUT_ROOT);
    fs.mkdirSync(outRoot, { recursive: true });
    fs.writeFileSync(path.join(outRoot, 'outskirtsExactP0Baseline.json'), `${JSON.stringify(report, null, 2)}\n`);
    fs.writeFileSync(path.join(outRoot, 'outskirtsExactP0Baseline.md'), `${toMarkdown(report)}\n`);
  }
