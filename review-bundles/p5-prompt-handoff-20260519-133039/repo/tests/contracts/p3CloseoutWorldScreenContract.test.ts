...
test('WorldScreen considers Run Compass secondary world-module routes independently of primary target kind', () => {
  const source = read('src/components/screens/WorldScreen.tsx');
  const block = source.slice(source.indexOf('const runCompassModuleKeys'), source.indexOf('const allowWorldRecommendationFallback'));

  assert.doesNotMatch(block, /const secondary = primaryTarget\?\.kind === 'world_module'\s*\?/u);
  assert.match(block, /runCompass\.v2\?\.secondaryRoutes/u);
});
