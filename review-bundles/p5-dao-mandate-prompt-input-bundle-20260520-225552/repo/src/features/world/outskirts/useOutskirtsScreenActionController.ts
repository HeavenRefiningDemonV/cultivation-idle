...
import type { OutskirtsSetupCard, OutskirtsSurfaceMode } from './types.js';

interface OutskirtsScreenActionControllerOptions {
  cityId: string;
  outskirtsDef: OutskirtsDef | undefined;
  activityMode: OutskirtsSurfaceMode;
  onStartOutskirts: () => void;
  onStopOutskirts: () => void;
  onOpenSettings: () => void;
...
export function useOutskirtsScreenActionController(options: OutskirtsScreenActionControllerOptions) {
  ...
  return {
    onPrimaryAction: options.activityMode === 'active' ? options.onStopOutskirts : options.onStartOutskirts,
